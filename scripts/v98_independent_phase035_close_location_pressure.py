...
def main():
    ...
    hyp="Persistent lagged candle close-location may measure directional auction pressure that predicts subsequent cross-sectional returns."; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase035_close_location_pressure_preregistration.md','selection_policy':'single frozen causal lagged-CLV specification; no sign/window/cadence/gross/threshold/regime search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
