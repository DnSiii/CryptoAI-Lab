...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase85_oi_contraction_taker_absorption_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';ALPHA_GROSS=.20
TAKER='sum_taker_long_short_vol_ratio';OI='sum_open_interest_value'
def get(u):
 last=None
 for attempt in range(5):
  try:
   req=urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase85'})
...
def load(job):
 ...
 prev=df['oi'].shift(1);adj=(df.index.to_series().diff()==pd.Timedelta(hours=1));growth=np.log(df['oi']/prev).where(adj);contraction=(-growth).clip(lower=0);raw=-np.log(df['taker'])*contraction;raw=raw.replace([np.inf,-np.inf],np.nan).dropna();return s,list(raw.items())
def main():
 prev=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase84_oi_confirmed_takerflow_train_alpha.json').read_text());assert prev['status']=='TRAIN_ALPHA_REJECT' and prev['precommitment']['holdout_not_parsed']
 ...
 out={'study':'V99 R106 Phase85 — native OI-CONTRACTION TAKER-ABSORPTION TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase85_oi_contraction_taker_absorption_prereg.md','source':'Binance USD-M daily metrics sum_taker_long_short_vol_ratio + sum_open_interest_value','transform':'[-log(taker_t) * max(-log(OI_t/OI_t-1),0)].shift(1)','direction':'contrarian_absorption','single_hypothesis_no_grid':True,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'adjacent_hour_oi_growth_required':True,'same_hour_pair_required':True},'train_end':train_end.isoformat(),'diagnostic':d,'selected_train_only':'native_oi_contraction_taker_absorption' if passed else None,'archives_consumed':len(jobs),'next_gate':'If pass, freeze exact specification and evaluate supersevere/regime/benchmark/reproducibility gates before untouched holdout; if reject, no retuning.','quarantined_symbols':quarantined,'disclosure':'Phase85 consumes native metrics only through train_end; every archive SHA256+CRC verified; taker and OI must be positive finite in same hour; OI contraction requires adjacent hours; interaction shifted t-1; no holdout metrics/returns inspected.'};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'archives_consumed':len(jobs),'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
