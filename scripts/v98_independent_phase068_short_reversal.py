...
CFG=PROJECT/'config'/'v98_independent.json';OUT=PROJECT/'reports'/'v98_independent_phase068_short_reversal.json';POS=PROJECT/'reports'/'v98_independent_phase068_short_reversal_positions.csv'
PREREG='reports/v98_independent_phase068_prereg.md';SYMBOLS=('BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT');GROSS=.75;LOOKBACK=3

def targets(data,start,end):
 close=data.close.loc[:end,list(SYMBOLS)];daily=close.resample('1D').last();score=daily.pct_change(LOOKBACK,fill_method=None).shift(1)
 ...
 for ts in t.index[events]:
  ...
  ordered=sorted(((float(s[k]),k) for k in SYMBOLS),key=lambda x:(x[0],x[1]));loser=ordered[0][1];winner=ordered[-1][1];t.loc[ts,loser]=GROSS/2;t.loc[ts,winner]=-GROSS/2
...
def main():
 ...
 poshash=hashlib.sha256(t.loc[a:b,list(SYMBOLS)].to_csv().encode()).hexdigest();cfghash=hashlib.sha256((PROJECT/PREREG).read_bytes()).hexdigest();rep={'engine':'V98 Independent','phase':'068','hypothesis':'frozen 3-day cross-sectional short-term reversal','preregistration':PREREG,'parameters':{'symbols':SYMBOLS,'lookback_days':LOOKBACK,'gross':GROSS,'rebalance':'00:00 UTC daily','long':'worst trailing return','short':'best trailing return'},'training':train,'folds':folds,'stress_training':stress,'regimes_training':p3.regime_metrics(base,data,a,b,b),'concentration_training':p3.concentration_metrics(base,a,b),'asset_contribution_proxy':contribution_proxy(data,t,a,b),'tails_training':tails(base,a,b),'reproducibility':{'positions_sha256':poshash,'prereg_sha256':cfghash},'training_gate':{'passed':not fail,'failures':fail},'validation':None,'final_holdout':None,'final_holdout_untouched':True,'v99_used':False,'v16_used':False,'parameter_search':False,'rescue_allowed':False};OUT.write_text(json.dumps(rep,indent=2,default=str)+'\n');base.positions.to_csv(POS);print(json.dumps({'phase':'068','training_gate':rep['training_gate'],'training':train,'folds':folds,'stress':stress,'final_holdout':'UNTOUCHED'},indent=2,default=str))
