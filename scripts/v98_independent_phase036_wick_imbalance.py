...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase036_wick_imbalance.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase036_wick_imbalance_positions.csv'
PHASE={'id':'phase_036_wick_imbalance','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'signal_lookback_hours':24,'min_signal_obs':18,'beta_lookback_hours':720,'min_beta_obs':360,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    c=PHASE; close=data.close; op=data.frames['open']; high=data.frames['high']; low=data.frames['low']; lagc=close.shift(1); lago=op.shift(1); lagh=high.shift(1); lagl=low.shift(1); rng=lagh-lagl
    upper=lagh-pd.DataFrame(np.maximum(lago.to_numpy(float),lagc.to_numpy(float)),index=close.index,columns=close.columns); lower=pd.DataFrame(np.minimum(lago.to_numpy(float),lagc.to_numpy(float)),index=close.index,columns=close.columns)-lagl
    imbalance=(lower-upper).div(rng.where(rng.abs()>1e-12)).fillna(0.0); score=imbalance.rolling(c['signal_lookback_hours'],min_periods=c['min_signal_obs']).mean()
    ...
    for k,ts in enumerate(targets.index):
        ...
        names=list(eligible.columns[eligible.loc[ts] & score.loc[ts].notna() & beta.loc[ts].notna()])
        if len(names)<4: continue
        s=score.loc[ts,names].rank(pct=True)-0.5; y=s.to_numpy(float); b=beta.loc[ts,names].to_numpy(float); X=np.column_stack([np.ones(len(names)),b]); coef,*_=np.linalg.lstsq(X,y,rcond=None); neutral=y-X@coef; gross=float(np.abs(neutral).sum())
...
def main():
    ...
    hyp='Persistent lagged lower-vs-upper wick imbalance may capture auction rejection pressure that predicts subsequent cross-sectional returns.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase036_wick_imbalance_preregistration.md','selection_policy':'single frozen causal lagged wick-imbalance specification; no sign/window/cadence/gross/threshold/regime search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
