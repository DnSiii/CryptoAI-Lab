...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase042_funding_shock_reversal.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase042_funding_shock_reversal_positions.csv'
PHASE={'id':'phase_042_funding_shock_reversal','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'min_beta_obs':360,'short_hours':24,'baseline_hours':168,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
 c=PHASE; close=data.close; lagc=close.shift(1); ret=lagc.pct_change(fill_method=None); btc=ret['BTCUSDT']; var=btc.rolling(c['beta_lookback_hours'],min_periods=c['min_beta_obs']).var(); beta=ret.rolling(c['beta_lookback_hours'],min_periods=c['min_beta_obs']).cov(btc).div(var,axis=0)
 known=data.funding.shift(1); short=known.rolling(c['short_hours'],min_periods=c['short_hours']).mean(); base=known.rolling(c['baseline_hours'],min_periods=c['baseline_hours']).mean(); score=-(short-base); eligible=(membership & close.notna()); eligible['BTCUSDT']=False
 ...
 for k,ts in enumerate(targets.index):
  ...
  y=(score.loc[ts,names].rank(pct=True)-0.5).to_numpy(float); b=beta.loc[ts,names].to_numpy(float); X=np.column_stack([np.ones(len(names)),b]); coef,*_=np.linalg.lstsq(X,y,rcond=None); neutral=y-X@coef; gross=float(np.abs(neutral).sum())
...
def main():
 ...
 hyp='A causal 24h funding-pressure shock versus a 168h baseline may capture crowded positioning reversal distinct from persistent funding-level carry.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase042_funding_shock_reversal_preregistration.md','selection_policy':'single frozen funding-shock reversal; no sign/window/cadence/gross/threshold/regime search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
 if passed:
