#!/usr/bin/env python3
"""V98 Independent Phase128: STLFSI4 DATA_ONLY feasibility. Never emits numeric series values."""
...
OUT = Path("reports/v98_independent_phase128_stl_fsi_data_only.json")
PREREG = Path("reports/v98_independent_phase128_stl_fsi_data_only_prereg.md")

def sha(b: bytes)->str: return hashlib.sha256(b).hexdigest()

def main():
    req=urllib.request.Request(URL, headers={"User-Agent":"CryptoAI-Lab-V98/phase128"})
    ...
    if status==200 and raw:
        try:
            ...
            required=bool(reader.fieldnames) and "DATE" in reader.fieldnames and "STLFSI4" in reader.fieldnames
            if required:
                for x in reader:
