...
def probe(asset: str, day: str) -> dict:
    q = urllib.parse.urlencode({"assets": asset, "metrics": ",".join(METRICS), "frequency": "1d", "start_time": day, "end_time": day, "page_size": 10})
    url = BASE + "?" + q
    try:
        ...
        present = {m: finite_value(row.get(m)) for m in METRICS}
        return {"ok": status == 200 and bool(rows) and all(present.values()), "http_status": status, "rows": len(rows), "time": row.get("time"), "metrics_present": present}
