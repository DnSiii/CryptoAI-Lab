...
REPORT = PROJECT / "reports" / "candidate_v99_r106_phase28_volatility_compression_breakout_alpha_audit.json"
ALPHA_GROSS = 0.20
TOP_N = 2
REBALANCE_HOURS = 24
VOL_LOOKBACK = 168
...
def fixed_sleeves(data) -> dict[str, pd.DataFrame]:
    ...
    rv24_med30d = rv24.rolling(24 * 30, min_periods=24 * 15).median().shift(1)
    compressed = rv24.lt(rv24_med30d)
    scale24 = rv24.mul(np.sqrt(24.0)).replace(0.0, np.nan)
    scale72 = rv24.mul(np.sqrt(72.0)).replace(0.0, np.nan)
    expansion24 = r24.abs().gt(scale24)
    expansion72 = r72.abs().gt(scale72)
    q24 = quote.rolling(24, min_periods=12).mean().shift(1)
    q168 = quote.rolling(168, min_periods=96).mean().shift(1)
    volume_confirm = q24.gt(q168)
    scores = {
        "compression_breakout_24h": r24.where(compressed & expansion24),
        "compression_breakout_72h": r72.where(compressed & expansion72),
        "compression_breakout_24h_volume_confirmed": r24.where(compressed & expansion24 & volume_confirm),
    }
...
def main():
    ...
    out={
      "study":"V99 R106 phase 28 — volatility-compression breakout alpha audit","status":"DIAGNOSTIC_ONLY_NO_STRATEGY_CHANGE",
      "frozen_assets_untouched":{"v16":True,"v99_frozen":True},
      "precommitment":{"objective":"test fixed volatility-compression breakout mechanisms after phase27 reversal failed","families":list(sleeves),"alpha_gross":ALPHA_GROSS,"top_n_each_side":TOP_N,"rebalance_hours":REBALANCE_HOURS,"market_neutral":True,"inverse_vol_weighted":True,"all_features_causal_t_minus_1":True,"selection_uses_train_only":True,"holdout_cannot_change_selected_family":True,"no_parameter_grid":True,"cost_for_selection":"severe","strategy_change_in_phase28":False,"next_candidate_allowed_only_if":"one train-selected sleeve is stable in >=3 eligible temporal folds and independently passes untouched holdout"},
      ...
      "sleeves":diagnostics,"selected_train_only":selected,"selected_holdout_descriptive":holdout,"selected_holdout_pass":holdout_pass,"actionable_for_phase29":bool(selected and holdout_pass),
      "next_phase_policy":"If actionable, phase29 may integrate only the exact train-selected sleeve and must run full severe+supersevere temporal-fold regime-matrix benchmark-envelope validation. If not actionable, reject compression breakout without horizon/grid tuning and move to a distinct mechanism.",
      "quarantined_symbols":quarantined,"metadata":metadata,"disclosure":"Historical research only. No real orders. Phase28 cannot alter F7/F9, V99 Frozen, V16 Frozen or paper state."}
    REPORT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+"\n",encoding="utf-8")
    print(json.dumps({"selected_train_only":selected,"selected_holdout_pass":holdout_pass,"actionable_for_phase29":out["actionable_for_phase29"],"train_summary":{n:{"stable_train":r["stable_train"],"healthy_folds":r["healthy_folds"],"train":r["train"]} for n,r in diagnostics.items()}},indent=2,default=audit.safe_float),flush=True)
