#!/usr/bin/env python3
"""V99 R106 Phase146 preregistered cross-venue body-intensity divergence reversion."""
...
PROJECT=Path(__file__).resolve().parents[1]; OUT=PROJECT/'reports'/'candidate_v99_r106_phase146_crossvenue_body_intensity_train_alpha.json'; PH136=PROJECT/'reports'/'candidate_v99_r106_phase136_okx_crossvenue_data_audit.json'; PREREG=PROJECT/'docs'/'v99_r106_phase146_crossvenue_body_intensity_divergence_preregister.md'
...
def fetch_json(params,retries=5):
    ...
    for attempt in range(retries):
        try:
            req=Request(BASE+'?'+urlencode(params),headers={'User-Agent':'CryptoAI-Lab-V99-R106-Phase146/1.0','Accept':'application/json','Connection':'close'})
...
def acquire(inst):
    ...
    vals=np.array([[float(x) for x in rows[t].split('|')[1:5]] for t in ts]); o,h,l,c=vals.T; valid=np.isfinite(vals).all(axis=1)&(o>0)&(h>=l); bi=np.full(len(ts),np.nan); bi[valid]=(c[valid]-o[valid])/o[valid]
    return pd.Series(bi,index=idx),digest,reqs,int(valid.sum())
...
def main():
    pt=PREREG.read_text(); assert LOOKBACK==168 and 'direction: reversion' in pt and 'exactly t-1' in pt and '>=98%' in pt
    ...
    aligned=okx_bi.notna()&bin_bi.notna()
    ...
    out={'study':'V99 R106 Phase146 — CROSS-VENUE BODY-INTENSITY DIVERGENCE REVERSION TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'lookback_hours':168,'direction':'reversion','selection_train_only':True,'no_sign_flip':True,'no_grid':True,'holdout_rows_used_for_feature_construction':0,'holdout_rows_used_for_selection':0},'source_integrity':{'all_phase136_hashes_reproduced':True,'hashes':hashes,'okx':integrity,'binance':bin_integrity,'imputation':False},'causality_invariants':{'complete_score_shift_hours':1,'post_train_targets_zero':True,'max_l1':float(targets.abs().sum(axis=1).max())},'diagnostic':d,'selected_train_only':'crossvenue_body_intensity_reversion_mad168' if passed else None,'next_gate':'PASS: severe/supersevere then regimes/tails/concentration/benchmark/reproducibility; FAIL: permanent, no tuning/sign flip.','quarantined_symbols':quarantined,'metadata':metadata}; OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n'); print(json.dumps({'status':out['status'],'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train'],'integrity':integrity},indent=2,default=audit.safe_float),flush=True)
