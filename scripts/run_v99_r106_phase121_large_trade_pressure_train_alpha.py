...
BASE='https://data.binance.vision/data/futures/um/daily/aggTrades'; TRAIN_END=pd.Timestamp('2024-01-18',tz='UTC'); TRAIN_END_MS=int(TRAIN_END.timestamp()*1000); N=12; MIN_ASSETS=8; GROSS=.20; WINDOW_MS=30*24*3600*1000; Q=.90; HOUR_MS=3600*1000
...
def raw_rows(s,d):
 ...
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  for row in csv.reader(io.TextIOWrapper(z.open(names[0]),encoding='utf-8')):
   ...
   if ts>=TRAIN_END_MS:raise RuntimeError('holdout timestamp encountered '+stem)
   yield ts,q,bm
...
def build_symbol(s,dates,index):
 ...
 rq=ExactRollingQuantile(support,WINDOW_MS,Q);out={};cur_hms=None;buy=sell=0.;hour_rows=[]
 # Pass 2 is chronological. query() occurs at each integer hour boundary BEFORE any trade from that hour is added.
 # Integer epoch arithmetic is semantically identical to floor('1h') but avoids constructing a Timestamp per raw trade.
 for d in dates:
  for ts,q,bm in raw_rows(s,d):
   hms=(ts//HOUR_MS)*HOUR_MS
   if cur_hms is None:cur_hms=hms;thr=rq.query(ts)
   elif hms!=cur_hms:
    if buy+sell>0:out[pd.Timestamp(cur_hms,unit='ms',tz='UTC')]=(buy-sell)/(buy+sell)
    for ets,eq in hour_rows:rq.add(ets,eq)
    cur_hms=hms;buy=sell=0.;hour_rows=[];thr=rq.query(ts)
   if thr is not None and q>thr:
    if bm=='false':buy+=q
    else:sell+=q
   hour_rows.append((ts,q))
 if cur_hms is not None:
  if buy+sell>0:out[pd.Timestamp(cur_hms,unit='ms',tz='UTC')]=(buy-sell)/(buy+sell)
  for ets,eq in hour_rows:rq.add(ets,eq)
