...
PROJECT=Path(__file__).resolve().parents[1]; OUT=PROJECT/'reports'/'candidate_v99_r106_phase144_crossvenue_close_location_train_alpha.json'; PH136=PROJECT/'reports'/'candidate_v99_r106_phase136_okx_crossvenue_data_audit.json'; PREREG=PROJECT/'docs'/'v99_r106_phase144_crossvenue_close_location_divergence_preregister.md'
...
def fetch_json(params,retries=5):
    ...
    for attempt in range(retries):
        try:
            req=Request(BASE+'?'+urlencode(params),headers={'User-Agent':'CryptoAI-Lab-V99-R106-Phase144/1.0','Accept':'application/json','Connection':'close'})
...
def acquire(inst):
    ...
    vals=np.array([[float(x) for x in rows[t].split('|')[1:5]] for t in ts]); o,h,l,c=vals.T; den=h-l; valid=np.isfinite(vals).all(axis=1)&(den>0); clv=np.full(len(ts),np.nan); clv[valid]=(c[valid]-o[valid])/den[valid]
    return pd.Series(clv,index=idx),digest,reqs,int(valid.sum())
...
def main():
    ...
    aligned=okx_clv.notna()&bin_clv.notna()
    ...
    out={'study':'V99 R106 Phase144 — CROSS-VENUE CLOSE-LOCATION DIVERGENCE REVERSION TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'lookback_hours':168,'direction':'reversion','selection_train_only':True,'no_sign_flip':True,'no_grid':True,'holdout_rows_used_for_feature_construction':0,'holdout_rows_used_for_selection':0},'source_integrity':{'all_phase136_hashes_reproduced':True,'hashes':hashes,'okx':integrity,'binance':bin_integrity,'zero_range_imputed':False},'causality_invariants':{'complete_score_shift_hours':1,'post_train_targets_zero':True,'max_l1':float(targets.abs().sum(axis=1).max())},'diagnostic':d,'selected_train_only':'crossvenue_close_location_reversion_mad168' if passed else None,'next_gate':'PASS: severe/supersevere then regimes/tails/concentration/benchmark/reproducibility; FAIL: permanent, no tuning/sign flip.','quarantined_symbols':quarantined,'metadata':metadata}; OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n'); print(json.dumps({'status':out['status'],'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train'],'integrity':integrity},indent=2,default=audit.safe_float),flush=True)
