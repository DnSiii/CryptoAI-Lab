...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase95_open_interest_change_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';FIELD='sum_open_interest_value';ALPHA_GROSS=.20

def get(u):
 last=None
 for attempt in range(5):
  try:
   req=urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase95'})
...
def main():
 assert (PROJECT/'research'/'v99_r106_phase95_open_interest_change_prereg.md').exists();prev=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase94_global_positioning_change_train_alpha.json').read_text());assert prev['status']=='TRAIN_ALPHA_REJECT' and prev['precommitment']['holdout_not_parsed']
 ...
 feature=f.diff(24).shift(1);targets=cs_weights(feature,data.close);severe=float(ex['severe_cost_per_side']);result=p1.run_targets(data,targets,ex,guard,severe,ALPHA_GROSS);train_end=pd.Timestamp(da['train_end'],tz='UTC');start=max(data.close.index[0],pd.Timestamp('2021-12-01',tz='UTC'));d=p47.diag(result,data.close.index,start,train_end);passed=bool(d['stable_train'])
 out={'study':'V99 R106 Phase95 — OPEN INTEREST 24H CHANGE TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase95_open_interest_change_prereg.md','source':'Binance USD-M daily metrics sum_open_interest_value','transform':'log OI value; 24h difference; shifted t-1; simultaneous median/MAD robust z; raw=+tanh(z); L1 normalized; min 10 assets','direction':'continuation in relative 24h open-interest change','single_hypothesis_no_grid':True,'change_horizon_hours':24,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'simultaneous_crosssection_required':True,'min_assets':10,'unit_tanh_scale_fixed':True},'train_end':train_end.isoformat(),'diagnostic':d,'selected_train_only':'native_open_interest_24h_change_continuation' if passed else None,'archives_consumed':len(jobs),'next_gate':'If pass, freeze exact specification and evaluate supersevere/regime/benchmark/reproducibility gates before untouched holdout; if reject, no retuning.','quarantined_symbols':quarantined,'disclosure':'Phase95 consumes native metrics only through train_end; every archive SHA256+CRC verified; positive finite OI value; 24h log change then feature shifted t-1; simultaneous median/MAD >=10 assets; fixed +tanh(z), no tuning; no holdout inspected.'};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'archives_consumed':len(jobs),'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
