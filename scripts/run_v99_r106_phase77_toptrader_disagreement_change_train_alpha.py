...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase77_toptrader_disagreement_change_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';ALPHA_GROSS=.20
FIELDS=('sum_toptrader_long_short_ratio','count_toptrader_long_short_ratio')
def get(u):
 last=None
 for attempt in range(5):
  try:
   req=urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase77'})
...
def load(job):
 ...
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  rd=csv.DictReader(io.TextIOWrapper(z.open(z.namelist()[0]),encoding='utf-8'));rows=[];need={'create_time',*FIELDS}
  if not need.issubset(set(rd.fieldnames or [])):raise RuntimeError('schema '+stem+': '+repr(rd.fieldnames))
  for r in rd:
   t=pd.to_datetime(r['create_time'],utc=True);a=(r.get(FIELDS[0]) or '').strip();b=(r.get(FIELDS[1]) or '').strip()
...
def main():
 ...
 out={'study':'V99 R106 Phase77 — native TOP-TRADER DISAGREEMENT CHANGE TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase77_toptrader_disagreement_change_prereg.md','source':'Binance USD-M daily metrics top-trader position/account ratios','transform':'diff(log(sum_toptrader_long_short_ratio/count_toptrader_long_short_ratio),1).shift(1)','direction':'continuation','single_hypothesis_no_grid':True,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'blank_or_nonfinite_observations_treated_missing':True,'missing_hours_not_bridged':True},'train_end':train_end.isoformat(),'diagnostic':d,'selected_train_only':'native_toptrader_disagreement_change_1h' if passed else None,'archives_consumed':len(jobs),'next_gate':'If pass, freeze exact specification and evaluate supersevere/regime/benchmark gates before untouched holdout; if reject, no sign/horizon/level/smoothing/threshold retuning.','quarantined_symbols':quarantined,'disclosure':'Phase77 consumes native metrics only through train_end; every archive SHA256+CRC verified; both fields required same hour; missing/nonfinite remain missing; adjacent-hour disagreement change then t-1 shift; no holdout metrics/returns inspected.'};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'archives_consumed':len(jobs),'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
