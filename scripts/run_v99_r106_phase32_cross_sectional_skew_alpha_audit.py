...
REPORT=PROJECT/'reports'/'candidate_v99_r106_phase32_cross_sectional_skew_alpha_audit.json'
ALPHA_GROSS=.20
...
def sleeves(data):
    ...
    out={}
    # Fixed low-skew anomaly: long names with lowest trailing return skew, short highest.
    # Rolling skew is shifted one full bar; decisions at t use only information <= t-1.
    for h in (72,168,336):
        sk=r.rolling(h,min_periods=max(48,h//2)).skew().shift(1)
        out[f'low_skew_{h}h']=p31.weights(-sk,c)
    return out
...
def main():
    ...
    out={'study':'V99 R106 phase 32 — cross-sectional skew alpha audit','status':'DIAGNOSTIC_ONLY_NO_STRATEGY_CHANGE','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'objective':'test fixed cross-sectional low-skew anomaly after phase31 low-volatility rejection; mechanism ranks higher moments rather than volatility or returns','families':list(ss),'alpha_gross':ALPHA_GROSS,'top_n_each_side':p31.TOP_N,'all_features_causal_t_minus_1':True,'selection_uses_train_only':True,'holdout_cannot_change_selected_family':True,'no_parameter_grid':True,'cost_for_selection':'severe','strategy_change_in_phase32':False},'data':{'common_start':start.isoformat(),'common_end':end.isoformat(),'train_end':train_end.isoformat(),'holdout_start':hold_start.isoformat(),'severe_cost_per_side':severe},'sleeves':ds,'selected_train_only':selected,'selected_holdout_descriptive':hold,'selected_holdout_pass':hp,'actionable_for_phase33':bool(selected and hp),'quarantined_symbols':quarantined,'metadata':metadata,'disclosure':'Historical research only. No real orders. Phase32 cannot alter V99 Frozen or V16 Frozen.'}
