...
def verify(key: str) -> dict:
    ...
    return {
        ...
        "bands": sorted(bands),
        "cadence_seconds_median": statistics.median(deltas) if deltas else None,
        "cadence_seconds_min": min(deltas) if deltas else None,
        "cadence_seconds_max": max(deltas) if deltas else None,
        "first_timestamp": unique_ts[0].isoformat(),
...
def main() -> None:
    ...
    for symbol, files in listed:
        if not files:
            per_symbol[symbol] = {
                ...
                "missing_dates": [], "coverage_ratio": 0.0,
            }
            continue
    ...
    for row in verified:
        ...
        bands.update(row["bands"])
        if row["cadence_seconds_median"] is not None:
            medians.append(float(row["cadence_seconds_median"]))

    out = {
        "study": "V99 R106 Phase114 — orthogonal Binance USD-M bookDepth DATA AUDIT ONLY",
        ...
        "object_listing_metadata_only": True,
        "source": {
            "listing": S3,
            "download": CDN + "data/futures/um/daily/bookDepth/<SYMBOL>/",
        ...
        "summary": {
            ...
            "bands": sorted(bands),
            "sample_median_cadence_seconds_min": min(medians) if medians else None,
            "sample_median_cadence_seconds_median": statistics.median(medians) if medians else None,
            "sample_median_cadence_seconds_max": max(medians) if medians else None,
        },
        "schema_inventory": [{"header": list(header), "sample_files": n} for header, n in schemas.items()],
        "integrity_sampling": {
            "method": "first available admitted archive of every calendar quarter per canonical symbol",
            "checks": [
                "SHA256", "ZIP CRC", "nonempty CSV", "required schema",
                "parseable monotone timestamps", "finite percentage/depth/notional",
                "nonnegative depth/notional", "unique timestamp/percentage rows",
            ],
            "samples": verified,
        },
        "alpha_prohibited": True,
        "return_relation_computed": False,
        "next_gate": "Review coverage/schema/cadence only. Any bookDepth alpha requires a separate post-audit preregistration before PnL.",
        "disclosure": "No order-book alpha, predictive relation, return/PnL comparison, regime selection or benchmark selection is computed in Phase114.",
    }
