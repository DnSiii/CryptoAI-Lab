...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase106_toptrader_size_divergence_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';SUM='sum_toptrader_long_short_ratio';COUNT='count_toptrader_long_short_ratio';ALPHA_GROSS=.20

def get(u):
 last=None
 for attempt in range(5):
  try:
   with urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase106'}),timeout=90) as r:return r.read()
...
def load(job):
 ...
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  if not {'create_time',SUM,COUNT}.issubset(set(rd.fieldnames or [])):raise RuntimeError('schema '+stem)
  for r in rd:
   try:t=pd.to_datetime(r['create_time'],utc=True);a=float(r[SUM]);b=float(r[COUNT])
...
def main():
 assert (PROJECT/'research'/'v99_r106_phase106_toptrader_size_divergence_prereg.md').exists();prev=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase105_toptrader_divergence_train_alpha.json').read_text());assert prev['status']=='TRAIN_ALPHA_REJECT' and prev['precommitment']['holdout_not_parsed']
 ...
 out={'study':'V99 R106 Phase106 — TOP-TRADER SIZE DIVERGENCE TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase106_toptrader_size_divergence_prereg.md','source':'Binance USD-M sum_toptrader_long_short_ratio + count_toptrader_long_short_ratio','transform':'median/MAD robust z of log(position-weighted toptrader ratio)-log(account-count toptrader ratio), whole alpha shifted t-1; tanh; L1 normalized','direction':'larger top-trader position tilt versus top-trader account-count tilt continuation','single_hypothesis_no_grid':True,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'simultaneous_crosssection_required':True,'min_assets':10,'unit_tanh_scale_fixed':True},'train_end':te.isoformat(),'diagnostic':d,'selected_train_only':'native_toptrader_size_divergence' if passed else None,'archives_consumed':len(jobs),'next_gate':'PASS freezes exact spec for supersevere/regime/benchmark/reproducibility before untouched holdout; FAIL permanent, no retuning.','quarantined_symbols':quarantined};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
