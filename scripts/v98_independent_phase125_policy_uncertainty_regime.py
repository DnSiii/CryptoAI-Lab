...
import v98_independent_phase124_policy_uncertainty_feasibility as p124
CFG=PROJECT/'config'/'v98_independent.json';OUT=PROJECT/'reports'/'v98_independent_phase125_policy_uncertainty_regime.json';POS=PROJECT/'reports'/'v98_independent_phase125_policy_uncertainty_regime_positions.csv'
PREREG='reports/v98_independent_phase125_policy_uncertainty_regime_prereg.md';GROSS=.75;ASSETS=['BTCUSDT','ETHUSDT','BNBUSDT','XRPUSDT','SOLUSDT'];LOOKBACK=28

def history():
 status,raw=p124.fetch_payload()
 if status!=200: raise RuntimeError(f'phase124 source unavailable: {status}')
 rows=[]
 for r in csv.DictReader(io.StringIO(raw.decode())):
  try:
   d=pd.Timestamp(r['observation_date'],tz='UTC');v=float(r['USEPUINDXD'])
   if np.isfinite(v) and pd.Timestamp('2023-01-01',tz='UTC')<=d<=pd.Timestamp('2025-12-31',tz='UTC'): rows.append((d,v))
  except Exception: pass
 s=pd.Series(dict(rows),name='epu').sort_index()
 if s.index.duplicated().any(): raise RuntimeError('duplicate EPU dates')
 return s,hashlib.sha256(raw).hexdigest()

def build_targets(data,s):
 med=s.rolling(LOOKBACK,min_periods=LOOKBACK).median();state=(s<=med).where(med.notna(),False)
 activation=(state.index+pd.Timedelta(days=2)).normalize();sig=pd.Series(state.astype(float).values,index=activation).sort_index()
 t=pd.DataFrame(np.nan,index=data.close.index,columns=data.close.columns);events=t.index.hour==0;states=[]
...
def main():
 gate=json.loads((PROJECT/'reports'/'v98_independent_phase124_policy_uncertainty_feasibility.json').read_text())
 if gate.get('decision')!='PASS_DATA_ONLY' or gate.get('pnl_computed') is not False: raise RuntimeError('Phase124 activation gate not satisfied')
