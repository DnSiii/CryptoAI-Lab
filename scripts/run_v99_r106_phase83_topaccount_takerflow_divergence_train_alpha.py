...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase83_topaccount_takerflow_divergence_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';ALPHA_GROSS=.20
TOP='count_toptrader_long_short_ratio';TAKER='sum_taker_long_short_vol_ratio'
def get(u):
 last=None
 for attempt in range(5):
  try:
   req=urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase83'})
...
def load(job):
 ...
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  rd=csv.DictReader(io.TextIOWrapper(z.open(z.namelist()[0]),encoding='utf-8'));rows=[];need={'create_time',TOP,TAKER}
  if not need.issubset(set(rd.fieldnames or [])):raise RuntimeError('schema '+stem+': '+repr(rd.fieldnames))
  for r in rd:
   t=pd.to_datetime(r['create_time'],utc=True);a=(r.get(TOP) or '').strip();b=(r.get(TAKER) or '').strip()
   if not a or not b:continue
   try:a=float(a);b=float(b)
   except ValueError:continue
   if not np.isfinite(a) or not np.isfinite(b) or a<=0 or b<=0:continue
   rows.append((t,np.log(a/b)))
...
def main():
 prev=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase82_topcapital_takerflow_divergence_train_alpha.json').read_text());assert prev['status']=='TRAIN_ALPHA_REJECT' and prev['precommitment']['holdout_not_parsed']
 ...
 out={'study':'V99 R106 Phase83 — native TOP-ACCOUNT vs TAKER-FLOW DIVERGENCE TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase83_topaccount_vs_takerflow_divergence_prereg.md','source':'Binance USD-M daily metrics count_toptrader_long_short_ratio + sum_taker_long_short_vol_ratio','transform':'log(count_toptrader_long_short_ratio/sum_taker_long_short_vol_ratio).shift(1)','direction':'continuation','single_hypothesis_no_grid':True,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'blank_or_nonfinite_observations_treated_missing':True,'same_hour_pair_required':True},'train_end':train_end.isoformat(),'diagnostic':d,'selected_train_only':'native_topaccount_vs_takerflow_divergence' if passed else None,'archives_consumed':len(jobs),'next_gate':'If pass, freeze exact specification and evaluate supersevere/regime/benchmark gates before untouched holdout; if reject, no sign/horizon/weight/smoothing/threshold retuning.','quarantined_symbols':quarantined,'disclosure':'Phase83 consumes native metrics only through train_end; every archive SHA256+CRC verified; both ratios must be positive finite in same hour; divergence shifted t-1; no holdout metrics/returns inspected.'};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'archives_consumed':len(jobs),'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
