...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase92_global_positioning_robust_distance_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';ALPHA_GROSS=.20;GLOBAL='count_long_short_ratio'
def get(u):
 last=None
 for attempt in range(5):
  try:
   req=urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase92'})
...
def main():
 assert (PROJECT/'research'/'v99_r106_phase91_cancellation.md').exists();prev=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase90_crosssectional_global_positioning_level_train_alpha.json').read_text());assert prev['status']=='TRAIN_ALPHA_REJECT' and prev['precommitment']['holdout_not_parsed']
 ...
 out={'study':'V99 R106 Phase92 — ROBUST GLOBAL POSITIONING DISTANCE TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase92_global_positioning_robust_distance_prereg.md','source':'Binance USD-M daily metrics count_long_short_ratio','transform':'log level shifted t-1; simultaneous median/MAD robust z; raw=-tanh(z); L1 normalized; min 10 assets','direction':'mean-reverting robust distance','single_hypothesis_no_grid':True,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'simultaneous_crosssection_required':True,'min_assets':10,'unit_tanh_scale_fixed':True},'train_end':train_end.isoformat(),'diagnostic':d,'selected_train_only':'native_global_positioning_robust_distance' if passed else None,'archives_consumed':len(jobs),'next_gate':'If pass, freeze exact specification and evaluate supersevere/regime/benchmark/reproducibility gates before untouched holdout; if reject, no retuning.','quarantined_symbols':quarantined,'disclosure':'Phase92 consumes native metrics only through train_end; every archive SHA256+CRC verified; positive finite ratio; feature shifted t-1; simultaneous median/MAD >=10 assets; degenerate MAD zero exposure; fixed -tanh(z), no tuning; no holdout inspected.'};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'archives_consumed':len(jobs),'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
