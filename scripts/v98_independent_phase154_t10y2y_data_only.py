#!/usr/bin/env python3
"""V98 Independent Phase154 T10Y2Y DATA_ONLY integrity auditor.
No crypto/economic relationship is inspected here.
"""
...
def weekdays(y):
 ...
 while d<=e:
  n+=d.weekday()<5; d+=timedelta(days=1)
 return n
def acquire():
 last=None
 for attempt in range(4):
  try:
   req=Request(URL,headers={"User-Agent":"CryptoAI-Lab-V98-Independent-Phase154/1.0","Accept":"text/csv","Connection":"close"})
   with urlopen(req,timeout=90) as r:return r.read()
  except Exception as exc:
   last=exc
   if attempt<3:time.sleep(2**attempt)
 raise RuntimeError(f"Phase154 transport failed after retries: {type(last).__name__}") from last
def audit(raw):
 reader=csv.DictReader(io.StringIO(raw.decode("utf-8-sig")))
 if reader.fieldnames != [DATE_FIELD,SERIES]: raise RuntimeError(f"unexpected schema: {reader.fieldnames!r}")
 obs=[]; malformed=missing=outside=weekend=0
 for row in reader:
  ...
  if d.weekday()>=5:weekend+=1;continue
 ...
 gates={"schema_identity":True,"unique_dates":duplicates==0,"strictly_increasing":ordered,"all_inside_frozen_window":outside==0,"weekdays_only":weekend==0,"global_coverage":total/total_expected>=MIN_COVERAGE,"year_coverage":all(coverage[str(y)]>=MIN_COVERAGE for y in YEARS),"no_malformed":malformed==0,"finite_signed_values":True}
 out={"phase":"154","mode":"DATA_ONLY","series":SERIES,"calendar":{"start":START.isoformat(),"end":END.isoformat(),"expected_basis":"weekdays_Mon_Fri"},"schema":{"date_field":DATE_FIELD,"value_field":SERIES},"counts":{"valid":total,"missing":missing,"malformed":malformed,"duplicates":duplicates,"outside_window":outside,"weekend_rows":weekend},"expected_weekdays":expected,"coverage":coverage,"global_coverage":total/total_expected,"normalized_observations_sha256":digest,"gates":gates,"decision":"PASS_DATA_ONLY" if all(gates.values()) else "REJECT_DATA_QUALITY_NO_RESCUE"}
