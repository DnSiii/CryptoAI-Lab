...
CFG=PROJECT/'config'/'v98_independent.json';OUT=PROJECT/'reports'/'v98_independent_phase084_sign_streak.json';POS=PROJECT/'reports'/'v98_independent_phase084_sign_streak_positions.csv'
PREREG='reports/v98_independent_phase084_prereg.md';SYMBOLS=('BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT');GROSS=.75;MAX_STREAK=7

def streak_score(close):
 d=close.resample('1D').last().pct_change(fill_method=None)
 out=pd.DataFrame(0.,index=d.index,columns=d.columns)
 for c in d.columns:
  prev=0
  vals=[]
  for x in d[c]:
   if not np.isfinite(x) or x==0: prev=0
   elif x>0: prev=min(MAX_STREAK,prev+1) if prev>0 else 1
   else: prev=max(-MAX_STREAK,prev-1) if prev<0 else -1
   vals.append(prev)
  out[c]=vals
 return out

def targets(data,start,end):
 close=data.close.loc[:end,list(SYMBOLS)];daily=streak_score(close)
 ...
 for ts in t.index[events]:
  ...
  if prior not in daily.index: continue
  s=daily.loc[prior,list(SYMBOLS)].dropna();ordered=sorted(((float(s[k]),k) for k in s.index),key=lambda x:(x[0],x[1]))
  if len(ordered)<2 or ordered[-1][0]<=0 or ordered[0][0]>=0: continue
...
def main():
 ...
 active=(t.loc[a:b,list(SYMBOLS)].abs().sum(axis=1)>0);rep={'engine':'V98 Independent','phase':'084','hypothesis':'frozen lagged completed-daily sign-streak persistence spread','preregistration':PREREG,'parameters':{'symbols':SYMBOLS,'max_streak_days':MAX_STREAK,'gross':GROSS,'rebalance':'00:00 UTC daily','magnitude_used':False},'phase083_selection_use':False,'future_holdout_required':True,'activation':{'active_hour_ratio':float(active.mean()),'active_hours':int(active.sum()),'hours':int(len(active))},'training':train,'folds':folds,'stress_training':stress,'regimes_training':p3.regime_metrics(base,data,a,b,b),'concentration_training':p3.concentration_metrics(base,a,b),'asset_contribution_proxy':contribution(data,t,a,b),'tails_training':tails(base,a,b),'reproducibility':{'positions_sha256':hashlib.sha256(t.loc[a:b,list(SYMBOLS)].to_csv().encode()).hexdigest(),'prereg_sha256':hashlib.sha256((PROJECT/PREREG).read_bytes()).hexdigest()},'training_gate':{'passed':not fail,'failures':fail},'validation':None,'final_holdout':None,'v99_used':False,'v16_used':False,'parameter_search':False,'rescue_allowed':False};OUT.write_text(json.dumps(rep,indent=2,default=str)+'\n');base.positions.to_csv(POS);print(json.dumps({'phase':'084','training_gate':rep['training_gate'],'training':train,'folds':folds,'stress':stress,'phase083_selection_use':False},indent=2,default=str))
