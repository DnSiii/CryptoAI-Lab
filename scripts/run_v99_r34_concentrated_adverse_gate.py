...
def features(raw,close):
 ...
 return share,sgn*r24,sgn*r72

def transform(raw,feat,p):
 share,signed24,signed72=feat
 ...
 t=raw*factor*p['gross_multiplier'];return cap_total(t,p['gross_cap']),bad

def slice_eval(data,raw,ex,guard,cg,bg,cost,p,start,end):
 d=sdata(data,start,end);rr=raw.reindex(index=d.close.index,columns=d.close.columns).fillna(0);tt,_,=transform(rr,features(rr,d.close),p);return stats(run(d,tt,ex,cost,cg,guard).equity),stats(run(d,rr,ex,cost,bg,guard).equity)
def main():
