...
def canonical_hourly(df: pd.DataFrame) -> pd.DataFrame:
    """Map 5m archive observations to the preregistered 1h information grid.

    This is acquisition/schema repair only. For each UTC hour use the last strictly
    valid, positive OI observation timestamped inside that hour. No interpolation,
    backfill, price data, future observation, or alpha-dependent choice is allowed.
    """
    x=pd.DataFrame({
        'timestamp':pd.to_datetime(df['create_time'],utc=True,errors='coerce'),
        'oi':pd.to_numeric(df['sum_open_interest'],errors='coerce'),
    })
    x=x[x['timestamp'].notna() & x['oi'].notna() & (x['oi']>0)].copy()
...
def one(symbol,date):
    ...
    try:
        ...
        oi=pd.to_numeric(df['sum_open_interest'],errors='coerce')
        dup=int(ts.duplicated().sum()); invalid_ts=int(ts.isna().sum())
        invalid_oi=int((~oi.notna() | (oi<=0)).sum())
        monotonic=bool(ts.dropna().is_monotonic_increasing)
        unique=ts.dropna().sort_values().unique()
        cadence_seconds=sorted(set(int(x.total_seconds()) for x in pd.Series(unique[1:]-unique[:-1]))) if len(unique)>1 else []
        hourly=canonical_hourly(df)
        expected=pd.date_range(pd.Timestamp(date,tz='UTC'),periods=24,freq='h')
        got=pd.DatetimeIndex(hourly['hour'])
        missing=[str(x) for x in expected.difference(got)]
        future_cross_hour=int((hourly['timestamp']>=hourly['hour']+pd.Timedelta(hours=1)).sum())
        hourly_ok=len(hourly)==24 and not missing and future_cross_hour==0
        return {'ok':dup==0 and invalid_ts==0 and monotonic and hourly_ok,
                ...
                'raw_invalid_or_nonpositive_oi':invalid_oi,'monotonic':monotonic,
                'cadence_seconds':cadence_seconds,'columns':list(map(str,df.columns)),
                'canonicalization':'last valid positive OI observed within each UTC hour; no interpolation/backfill',
                'canonical_hourly_rows':int(len(hourly)),'canonical_missing_hours':missing,
                'canonical_future_cross_hour_violations':future_cross_hour,
                'raw_anomalies_tolerated_only_if_hour_has_prior_valid_observation':True}
...
def main():
    ...
    schemas=sorted({tuple(v.get('columns',[])) for v in checks.values() if 'columns' in v})
    raw_anomaly_files=sum(int(v.get('raw_invalid_or_nonpositive_oi',0)>0) for v in checks.values())
    report={'engine':'V98 Independent','phase':'047','purpose':'OI acquisition/integrity audit only; no alpha returns, validation, or final holdout access',
            ...
            'canonical_grid':'1h UTC, last valid positive observation within each hour',
            'checks':checks,'ok_count':ok,'total_count':len(checks),'schema_count':len(schemas),
            'raw_anomaly_files':raw_anomaly_files,'column_sets':[list(x) for x in schemas],
            'decision':'integrity_sample_pass' if ok==len(checks) and len(schemas)==1 else 'block_phase047_backtest'}
    OUT.write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps({k:report[k] for k in ('ok_count','total_count','schema_count','raw_anomaly_files','decision')},indent=2))
