#!/usr/bin/env python3
"""V98 Independent Phase149 WTI DATA_ONLY integrity auditor."""
from __future__ import annotations
import csv, hashlib, io, json, sys, time
from decimal import Decimal, InvalidOperation
from datetime import date, datetime
from urllib.request import Request, urlopen
SERIES="DCOILWTICO"; START=date(2023,1,1); END=date(2025,12,31)
URL="https://fred.stlouisfed.org/graph/fredgraph.csv?id=DCOILWTICO&cosd=2023-01-01&coed=2025-12-31"
...
def acquire():
 ...
 for attempt in range(4):
  try:
   req=Request(URL,headers={"User-Agent":"CryptoAI-Lab-V98-Independent-Phase149/1.0","Accept":"text/csv","Connection":"close"})
   with urlopen(req,timeout=90) as r:return r.read()
  except Exception as exc:
   last=exc
   if attempt<3:time.sleep(2**attempt)
 raise RuntimeError(f"Phase149 transport failed after retries: {type(last).__name__}") from last
def audit(raw):
