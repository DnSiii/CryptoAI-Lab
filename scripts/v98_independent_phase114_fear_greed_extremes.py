...
import v98_independent_phase113_fear_greed_feasibility as p113
CFG=PROJECT/'config'/'v98_independent.json'; OUT=PROJECT/'reports'/'v98_independent_phase114_fear_greed_extremes.json'; POS=PROJECT/'reports'/'v98_independent_phase114_fear_greed_extremes_positions.csv'
PREREG='reports/v98_independent_phase114_fear_greed_extremes_prereg.md'; GROSS=.75; FEAR=25; GREED=75
ASSETS=['BTCUSDT','ETHUSDT','BNBUSDT','XRPUSDT','SOLUSDT']

def history():
    status,raw,err=p113.get(p113.URL)
    if status!=200: raise RuntimeError(f'phase113 source unavailable: {status} {err}')
    obj=json.loads(raw); rows=[]
    for x in obj.get('data',[]):
        try:
            ts=pd.to_datetime(int(x['timestamp']),unit='s',utc=True); val=int(x['value'])
            if 0<=val<=100: rows.append((ts,ts.normalize(),val))
        except Exception: pass
    df=pd.DataFrame(rows,columns=['timestamp','date','value']).sort_values('timestamp').drop_duplicates('date',keep='last')
    s=pd.Series(df.value.values,index=df.date,name='fng').sort_index()
    return s,hashlib.sha256(raw).hexdigest()

def build_targets(data,s):
    lag=s.shift(1); t=pd.DataFrame(0.,index=data.close.index,columns=data.close.columns); events=t.index.hour==0; states=[]
    for ts in t.index[events]:
        v=lag.get(ts.normalize(),np.nan)
        if not np.isfinite(v): continue
        if v<=FEAR: side=1.; state='fear'
        elif v>=GREED: side=-1.; state='greed'
        else: side=0.; state='neutral'
        t.loc[ts,ASSETS]=side*GROSS/len(ASSETS); states.append((ts.normalize(),int(v),state))
    return t.where(pd.Series(events,index=t.index),np.nan).ffill().fillna(0.),pd.DataFrame(states,columns=['date','lagged_value','state'])
...
def main():
    gate=json.loads((PROJECT/'reports'/'v98_independent_phase113_fear_greed_feasibility.json').read_text())
    if gate.get('gate')!='PASS_DATA_ONLY' or gate.get('alpha_or_pnl_computed') is not False: raise RuntimeError('Phase113 activation gate not satisfied')
    ...
    con=contribution(data,t,a,b); tail=tails(base,a,b); st=states[(states.date>=pd.Timestamp(a))&(states.date<=pd.Timestamp(b))].state.value_counts().to_dict(); op=base.open_positions.loc[a:b].abs().sum(axis=1) if base.open_positions is not None else pd.Series(dtype=float); maxopen=float(op.max()) if len(op) else 0.; benchmark=p3.metrics(ev(data,always_long(data),cfg,'base'),a,b)
    fail=[]
