...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase034_cross_asset_leadlag.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase034_cross_asset_leadlag_positions.csv'
PHASE={'id':'phase_034_cross_asset_leadlag','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'model_lookback_hours':720,'min_model_obs':360,'rebalance_hours':6,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    ...
    ret=lag.pct_change(fill_method=None); btc=ret['BTCUSDT']; var=btc.rolling(c['model_lookback_hours'],min_periods=c['min_model_obs']).var(); beta=ret.rolling(c['model_lookback_hours'],min_periods=c['min_model_obs']).cov(btc).div(var,axis=0)
    targets=pd.DataFrame(0.0,index=close.index,columns=close.columns); events=np.arange(len(targets))%c['rebalance_hours']==0
    for k,ts in enumerate(targets.index):
        if not events[k] or k<c['model_lookback_hours']+2: continue
        names=list(eligible.columns[eligible.loc[ts] & beta.loc[ts].notna()]); peers=list(membership.columns[membership.loc[ts] & ret.loc[ts].notna()])
        if len(names)<4 or len(peers)<4: continue
        lo=max(1,k-c['model_lookback_hours']); hist=ret.iloc[lo:k+1]; scores={}
        current=ret.iloc[k]
        for j in names:
            ps=[x for x in peers if x!=j and x in hist.columns]
            if len(ps)<3: continue
            y=hist[j].iloc[1:]; X=hist[ps].shift(1).iloc[1:]; z=pd.concat([y.rename('y'),X],axis=1).dropna()
            if len(z)<c['min_model_obs']: continue
            xv=z[ps].to_numpy(float); yv=z['y'].to_numpy(float); A=np.column_stack([np.ones(len(z)),xv]); coef,*_=np.linalg.lstsq(A,yv,rcond=None)
            now=current[ps].to_numpy(float)
            if np.isfinite(now).all() and np.isfinite(coef).all(): scores[j]=float(now@coef[1:])
        if len(scores)<4: continue
        s=pd.Series(scores); y=(s.rank(pct=True)-0.5).to_numpy(float); b=beta.loc[ts,s.index].to_numpy(float); X=np.column_stack([np.ones(len(s)),b]); coef,*_=np.linalg.lstsq(X,y,rcond=None); neutral=y-X@coef; gross=float(np.abs(neutral).sum())
...
def main():
    ...
    hyp='Lagged peer-asset returns may contain causal cross-sectional information through a rolling multivariate lead-lag network beyond BTC-only propagation.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase034_cross_asset_leadlag_preregistration.md','selection_policy':'single frozen causal cross-asset lead-lag specification; no sign/window/cadence/gross/subset/regime search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
    ...
    REPORT_PATH.parent.mkdir(parents=True,exist_ok=True); REPORT_PATH.write_text(json.dumps(report,indent=2,default=str)+'\n'); r['base'].positions.to_csv(POSITIONS_PATH); validation_pass=bool(passed and report['validation'] and report['validation']['gate']['passed']); state['phase']='phase_034_complete'; state['last_experiment']={'id':PHASE['id'],'hypothesis':hyp,'status':'frozen_pending_final_holdout' if validation_pass else ('training_passed_validation_rejected' if passed else 'training_rejected'),'parameters':'preregistered 720h multivariate peer lead-lag OLS; min 360 obs; 6h rebalance; 0.75 gross; beta/dollar neutral','report':str(REPORT_PATH.relative_to(PROJECT)),'training_summary':train,'folds':folds,'stress_training':report['stress_training'],'training_gate':report['training_gate'],'validation':report['validation'],'final_holdout_opened':False}; state['champion']=PHASE['id'] if validation_pass else state.get('champion'); state['final_holdout_untouched']=True; state['next_action']='If Phase034 passes training+validation, freeze before one-shot final holdout. If rejected, close cross-asset lead-lag without sign/window/cadence/subset rescue and move to another orthogonal alpha.'; STATE_PATH.write_text(json.dumps(state,indent=2,default=str)+'\n'); print(json.dumps({'phase':PHASE['id'],'training_gate':report['training_gate'],'training':train,'folds':folds,'stress':report['stress_training'],'validation':report['validation'],'final_holdout':'UNTOUCHED'},indent=2,default=str))
