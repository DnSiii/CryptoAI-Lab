...
import v98_independent_phase145_vix_data_only as p145
CFG=PROJECT/'config'/'v98_independent.json';PRE=PROJECT/'reports'/'v98_independent_phase146_vix_risk_off_preregistration.json';P145=PROJECT/'reports'/'v98_independent_phase145_vix_data_only.json';OUT=PROJECT/'reports'/'v98_independent_phase146_vix_risk_off.json'
ASSETS=['BTCUSDT','ETHUSDT','BNBUSDT','XRPUSDT','SOLUSDT'];LOOKBACK=20;LAG_DAYS=1;TARGET=.30;CAP=.35

def load_macro():
 raw=p145.acquire(); a=p145.audit(raw); governing=json.loads(P145.read_text()); assert governing['decision']=='PASS_DATA_ONLY'; assert a['normalized_observations_sha256']==governing['normalized_observations_sha256']
 r=csv.DictReader(io.StringIO(raw.decode('utf-8-sig'))); obs=[]
 for row in r:
  v=(row.get('VIXCLS') or '').strip()
...
def main():
 pre=json.loads(PRE.read_text()); assert pre['phase']=='146' and pre['anti_overfit']['parameter_search'] is False and pre['anti_overfit']['rescue_allowed'] is False
 s,h=load_macro(); assert h==pre['dependency']['phase145_observations_sha256']
