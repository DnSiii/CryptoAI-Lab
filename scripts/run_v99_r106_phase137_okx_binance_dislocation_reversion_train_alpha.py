...
def main():
    prereg=PROJECT/'research'/'v99_r106_phase137_okx_binance_dislocation_reversion_prereg.md';assert prereg.exists();txt=prereg.read_text()
    ...
    spread=np.log(bclose/okx_close);med=spread.rolling(LOOKBACK,min_periods=LOOKBACK).median();dev=spread-med
    mad=rolling_exact_mad(spread,LOOKBACK)
    scale=(1.4826*mad).where(mad>1e-12);z=dev/scale;raw_score=-np.tanh(z);centered=raw_score.sub(raw_score.mean(axis=1),axis=0);score=centered.shift(1)
    ...
    out={'study':'V99 R106 Phase137 — OKX/BINANCE CROSS-VENUE DISLOCATION REVERSION TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase137_okx_binance_dislocation_reversion_prereg.md','source':'OKX public 1H USDT-SWAP + canonical Binance USD-M closes','feature':'log(Binance/OKX) minus rolling median168, divided by 1.4826*exact same-window MAD168; -tanh; cross-sectional demean; complete score t-1; L1','mad_definition':'median(abs(window - median(window))) on the same 168h spread window','direction':'cross_venue_dislocation_mean_reversion','lookback_hours':LOOKBACK,'alpha_gross':ALPHA_GROSS,'frozen_symbols':list(MAP.keys()),'single_hypothesis_no_grid':True,'selection_train_only':True,'no_sign_flip':True,'no_symbol_substitution':True,'canonical_replay_may_materialize_full_panel':True,'holdout_rows_used_for_feature_construction':0,'holdout_rows_used_for_selection':0},'source_integrity':{'phase136_report':'reports/candidate_v99_r106_phase136_okx_crossvenue_data_audit.json','all_phase136_hashes_reproduced':True,'normalized_full_rows_sha256':hashes,'requests':requests,'okx_rows_per_instrument':len(okx_close),'binance_training_integrity':binance_integrity,'missing_binance_prices_filled':False,'minimum_binance_coverage':0.98},'train_start':TRAIN_START.isoformat(),'train_end_exclusive':TRAIN_END.isoformat(),'causality_invariants':{'complete_score_shift_hours':1,'feature_input_max_timestamp':okx_close.index.max().isoformat(),'feature_inputs_strictly_pre_train_end':bool(okx_close.index.max()<TRAIN_END),'post_train_targets_zero':True,'max_l1':float(targets.abs().sum(axis=1).max()),'common_venue_component_removed_by_cross_sectional_demean':True},'diagnostic':d,'selected_train_only':'okx_binance_dislocation_reversion_168h' if passed else None,'next_gate':'PASS freezes exact spec for supersevere/regimes/tails/concentration/benchmark/reproducibility before untouched holdout; FAIL permanent, no sign flip or lookback rescue.','quarantined_symbols':quarantined,'metadata':metadata}
