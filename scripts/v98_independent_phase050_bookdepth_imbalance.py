...
def gate_or_diagnostic(train,folds,sev,sup,conc):
 required=('total_return','max_drawdown','profit_factor_daily','payoff_daily','win_rate_daily','positive_days','worst_day','ruin')
 missing={name:[k for k in required if k not in m] for name,m in [('training',train),*folds.items(),('severe',sev),('supersevere',sup)]}
 missing={k:v for k,v in missing.items() if v}
 if missing:return False,['insufficient_evaluable_metrics:'+json.dumps(missing,sort_keys=True)]
 return p3.training_gate(train,folds,sev,sup,conc)
def main():
 ...
 liquid,membership=point_in_time_liquid_view(data,top_n=PHASE['liquidity_top_n'],lookback_hours=PHASE['liquidity_lookback_hours'],minimum_history_hours=PHASE['minimum_history_hours']); signal,cov=acquire(cfg['research_start'],cfg['training_end']); t=build_targets(liquid,membership,signal); r={s:ev(liquid,t,cfg,s) for s in ('base','severe','supersevere')}; a,b=cfg['research_start'],cfg['training_end']; train=p3.metrics(r['base'],a,b); folds={f['name']:p3.metrics(r['base'],f['start'],f['end']) for f in cfg['folds']}; sev=p3.metrics(r['severe'],a,b); sup=p3.metrics(r['supersevere'],a,b); conc=p3.concentration_metrics(r['base'],a,b); beta_n=p3.beta_neutrality_metrics(r['base'],liquid,a,b); regimes=p3.regime_metrics(r['base'],liquid,a,b,b); passed,failures=gate_or_diagnostic(train,folds,sev,sup,conc)
 diagnostics={'data_start':str(liquid.close.index.min()),'data_end':str(liquid.close.index.max()),'signal_start':str(signal.index.min()) if len(signal) else None,'signal_end':str(signal.index.max()) if len(signal) else None,'signal_rows':int(len(signal)),'nonzero_target_hours':int((t.abs().sum(axis=1)>1e-12).sum()),'max_target_gross':float(t.abs().sum(axis=1).max()) if len(t) else 0.0,'base_equity_nonnull':int(r['base'].equity.notna().sum())}
 hyp='Contracts with stronger prior-completed-day bid-vs-ask quoted notional imbalance subsequently outperform peers after dollar/BTC-beta neutralization.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase050_bookdepth_preregistration.md','acquisition_training':cov,'diagnostics':diagnostics,'selection_policy':'single frozen positive previous-completed-UTC-day median all-level book-depth imbalance; no sign/level/window/cadence/gross/threshold/regime/subset search','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
 if passed:
