...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase046_peer_correlation.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase046_peer_correlation_positions.csv'
PHASE={'id':'phase_046_peer_correlation','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'min_beta_obs':360,'corr_hours':168,'min_corr_obs':120,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
 c=PHASE; close=data.close; ret=close.shift(1).pct_change(fill_method=None); btc=ret['BTCUSDT']; var=btc.rolling(c['beta_lookback_hours'],min_periods=c['min_beta_obs']).var(); beta=ret.rolling(c['beta_lookback_hours'],min_periods=c['min_beta_obs']).cov(btc).div(var,axis=0); resid=ret-beta.mul(btc,axis=0); alt=resid.drop(columns=['BTCUSDT']); scores=pd.DataFrame(index=close.index,columns=close.columns,dtype=float); total=alt.sum(axis=1,min_count=2); count=alt.notna().sum(axis=1)
 for s in alt.columns:
  peer=(total-alt[s]).div((count-alt[s].notna().astype(int)).replace(0,np.nan)); scores[s]=-alt[s].rolling(c['corr_hours'],min_periods=c['min_corr_obs']).corr(peer)
 ...
 for k,ts in enumerate(targets.index):
  ...
  names=list(eligible.columns[eligible.loc[ts] & scores.loc[ts].notna() & beta.loc[ts].notna()])
  if len(names)<4: continue
  y=(scores.loc[ts,names].rank(pct=True)-0.5).to_numpy(float); b=beta.loc[ts,names].to_numpy(float); X=np.column_stack([np.ones(len(names)),b]); coef,*_=np.linalg.lstsq(X,y,rcond=None); neutral=y-X@coef; gross=float(np.abs(neutral).sum())
...
def main():
 ...
 hyp='Lower causal correlation of BTC-residual returns with a leave-one-out peer residual basket may proxy less crowded altcoin exposure and predict better relative performance.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase046_peer_correlation_preregistration.md','selection_policy':'single frozen peer-correlation crowding signal; no sign/window/cadence/gross/threshold/regime search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
 if passed:
