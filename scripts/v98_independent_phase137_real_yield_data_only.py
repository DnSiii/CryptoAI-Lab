#!/usr/bin/env python3
"""V98 Independent Phase137: US 10Y real-yield DATA_ONLY feasibility. Never emits observation values."""
...
OUT=Path("reports/v98_independent_phase137_real_yield_data_only.json")
...
def fetch_payload():
    ...
    raise RuntimeError(f"Phase137 source transport failed after deterministic retries: {type(last).__name__}") from last
...
def main():
    ...
    report={"engine":"V98 Independent","phase":"137","purpose":"US 10Y real yield DATA_ONLY feasibility; ZERO VALUES/ZERO ALPHA/ZERO RETURNS/ZERO PNL","source":"FRED public CSV","series_id":SERIES,"source_url":URL,"http_status":status,"training_only":True,"training_start":START.isoformat(),"training_end":END.isoformat(),"validation_accessed":False,"final_holdout_accessed":False,"v16_used":False,"v99_used":False,"phase083_selection_use":False,"values_exposed":False,"descriptives_computed":False,"returns_computed":False,"correlations_computed":False,"alpha_computed":False,"pnl_computed":False,"parameter_search":False,"rescue_allowed":False,"raw_rows":len(rows),"expected_weekdays":len(exp),"usable_weekdays":len(usable),"coverage":coverage,"missing_weekdays":len(exp)-len(usable),"source_calendar_missing_values":missing_values,"malformed_date_records":malformed_dates,"postcanonical_duplicate_dates":dup,"outside_training_rows":outside,"canonicalization":"parse ISO observation_date; retain finite DFII10 observations in frozen training window; source-calendar missing values counted separately; no fill/interpolation/imputation","payload_sha256":hashlib.sha256(payload).hexdigest(),"canonical_rows_sha256":canonical_hash,"gate":">=95% weekday coverage; zero malformed dates; zero duplicate finite dates; zero finite outside-training rows; deterministic hashes","decision":decision}
