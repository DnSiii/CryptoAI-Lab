...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase025_liquidity_efficiency.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase025_liquidity_efficiency_positions.csv'
PHASE={'id':'phase_025_liquidity_efficiency','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'impact_lookback_hours':720,'rebalance_hours':48,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    """Implements the Phase025 preregistration committed before Phase024 result harvest. Signal is low lagged price impact per lagged quote-volume, with fixed 30d windows; no grid/sign tuning."""
    ...
    qv=data.frames['quote_volume'].shift(1).clip(lower=1.0); impact=hourly.abs().div(qv); efficiency=-impact.rolling(c['impact_lookback_hours'],min_periods=360).median(); score=efficiency.rank(axis=1,pct=True).sub(0.5).where(eligible)
...
def main():
    ...
    hyp='Low causal price impact per lagged quote-volume may proxy liquidity efficiency and structural quality after dollar/BTC-beta neutralization.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase025_preregistration.md','selection_policy':'implemented exactly after preregistration committed before Phase024 result harvest; fixed 30d beta + 30d rolling-median impact; no grid/sign search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
