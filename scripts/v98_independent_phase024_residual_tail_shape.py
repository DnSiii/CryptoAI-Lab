...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase024_residual_tail_shape.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase024_residual_tail_shape_positions.csv'
PHASE={'id':'phase_024_residual_tail_shape','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'tail_lookback_hours':720,'rebalance_hours':48,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    """Pre-registered after Phase023 rejection. Orthogonal distribution-shape hypothesis: after removing causal BTC beta, assets with less leptokurtic residual returns may earn a cross-sectional quality premium because extreme idiosyncratic jump exposure is less fragile. Fixed 30d beta/tail windows, 48h rebalance, no grid/sign search; all features use t-1 or earlier."""
    ...
    residual=hourly.sub(beta.mul(btc,axis=0)); tail=residual.rolling(c['tail_lookback_hours'],min_periods=360).kurt(); score=(-tail).rank(axis=1,pct=True).sub(0.5).where(eligible)
...
def main():
    ...
    hyp='Low residual excess-kurtosis may proxy idiosyncratic tail quality after BTC-beta neutralization, producing a cross-sectional premium distinct from residual skew, volatility, autocorrelation, and beta stability.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'selection_policy':'single pre-registered residual-tail-shape architecture after Phase023 rejection; fixed 30d beta + 30d residual kurtosis; no grid/sign search; training/folds/stress first; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
    ...
    REPORT_PATH.parent.mkdir(parents=True,exist_ok=True); REPORT_PATH.write_text(json.dumps(report,indent=2,default=str)+'\n'); r['base'].positions.to_csv(POSITIONS_PATH); validation_pass=bool(passed and report['validation'] and report['validation']['gate']['passed']); state['phase']='phase_024_complete'; state['last_experiment']={'id':PHASE['id'],'hypothesis':hyp,'status':'frozen_pending_final_holdout' if validation_pass else ('training_passed_validation_rejected' if passed else 'training_rejected'),'parameters':'30d BTC beta + 30d residual kurtosis; 48h rebalance; 0.75 gross; beta/dollar neutral; no grid/sign search','report':str(REPORT_PATH.relative_to(PROJECT)),'training_summary':train,'folds':folds,'stress_training':report['stress_training'],'training_gate':report['training_gate'],'validation':report['validation'],'final_holdout_opened':False}; state['champion']=PHASE['id'] if validation_pass else state.get('champion'); state['final_holdout_untouched']=True; state['next_action']='If Phase024 passes training+validation, freeze before one-shot final holdout. If rejected, close residual-tail-shape family without window/sign tuning and move to another orthogonal alpha.'; STATE_PATH.write_text(json.dumps(state,indent=2,default=str)+'\n'); print(json.dumps({'phase':PHASE['id'],'training_gate':report['training_gate'],'training':train,'folds':folds,'stress':report['stress_training'],'validation':report['validation'],'final_holdout':'UNTOUCHED'},indent=2,default=str))
