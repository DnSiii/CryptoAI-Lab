...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase033_beta_asymmetry.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase033_beta_asymmetry_positions.csv'
PHASE={'id':'phase_033_beta_asymmetry','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'exposure_lookback_hours':720,'min_subset_obs':180,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
def _beta(y,x):
    z=pd.concat([y,x],axis=1).dropna()
    if len(z)<PHASE['min_subset_obs']: return np.nan
    xv=z.iloc[:,1].to_numpy(float); yv=z.iloc[:,0].to_numpy(float); var=float(np.var(xv,ddof=1))
    return float(np.cov(yv,xv,ddof=1)[0,1]/var) if np.isfinite(var) and var>1e-14 else np.nan
def build_targets(data,membership):
    c=PHASE; close=data.close; lag=close.shift(1); eligible=(membership & close.notna()).copy(); eligible['BTCUSDT']=False; ret=lag.pct_change(fill_method=None); btc=ret['BTCUSDT']; var=btc.rolling(c['exposure_lookback_hours'],min_periods=360).var(); beta=ret.rolling(c['exposure_lookback_hours'],min_periods=360).cov(btc).div(var,axis=0)
    ...
    for i,ts in enumerate(targets.index):
        ...
        lo=max(0,i-c['exposure_lookback_hours']+1); idx=ret.index[lo:i+1]; names=eligible.columns[eligible.loc[ts] & beta.loc[ts].notna()]; scores={}
        xb=btc.loc[idx]; up=xb>0; dn=xb<0
        for n in names:
            bu=_beta(ret.loc[idx,n].loc[up],xb.loc[up]); bd=_beta(ret.loc[idx,n].loc[dn],xb.loc[dn])
            if np.isfinite(bu) and np.isfinite(bd): scores[n]=bu-bd
...
def main():
    ...
    hyp='Lagged BTC upside-minus-downside beta asymmetry may contain cross-sectional return information beyond unconditional beta after dollar/beta neutralization.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase033_beta_asymmetry_preregistration.md','selection_policy':'single frozen causal beta-asymmetry specification; no sign/window/cadence/gross/threshold/regime search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
    ...
    REPORT_PATH.parent.mkdir(parents=True,exist_ok=True); REPORT_PATH.write_text(json.dumps(report,indent=2,default=str)+'\n'); r['base'].positions.to_csv(POSITIONS_PATH); validation_pass=bool(passed and report['validation'] and report['validation']['gate']['passed']); state['phase']='phase_033_complete'; state['last_experiment']={'id':PHASE['id'],'hypothesis':hyp,'status':'frozen_pending_final_holdout' if validation_pass else ('training_passed_validation_rejected' if passed else 'training_rejected'),'parameters':'preregistered 720h BTC upside/downside conditional betas; 180 subset obs; 24h rebalance; 0.75 gross; unconditional-beta/dollar neutral','report':str(REPORT_PATH.relative_to(PROJECT)),'training_summary':train,'folds':folds,'stress_training':report['stress_training'],'training_gate':report['training_gate'],'validation':report['validation'],'final_holdout_opened':False}; state['champion']=PHASE['id'] if validation_pass else state.get('champion'); state['final_holdout_untouched']=True; state['next_action']='If Phase033 passes training+validation, freeze before one-shot final holdout. If rejected, close beta-asymmetry without sign/window/cadence rescue and move to another orthogonal alpha.'; STATE_PATH.write_text(json.dumps(state,indent=2,default=str)+'\n'); print(json.dumps({'phase':PHASE['id'],'training_gate':report['training_gate'],'training':train,'folds':folds,'stress':report['stress_training'],'validation':report['validation'],'final_holdout':'UNTOUCHED'},indent=2,default=str))
