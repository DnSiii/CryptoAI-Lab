...
BASE='https://data.binance.vision/data/futures/um/daily/aggTrades'; ALPHA_GROSS=.20; N=12; MIN_ASSETS=8; TRAIN_END=pd.Timestamp('2024-01-18',tz='UTC'); HOUR_MS=3_600_000
...
def load(job):
 ...
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  for row in csv.reader(io.TextIOWrapper(z.open(names[0]),encoding='utf-8')):
   ...
   hour_ms=(ts//HOUR_MS)*HOUR_MS
   a=acc.setdefault(hour_ms,[0,0])
   if bm=='false':a[0]+=1
   else:a[1]+=1
 return s,[(hour_ms,b,sl) for hour_ms,(b,sl) in acc.items()]
...
def main():
 ...
 x=pd.DataFrame(index=data.close.index,columns=common,dtype=float); index_ms={int(t.value//1_000_000):t for t in x.index if t<TRAIN_END}
 with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
  for s,rows in pool.map(load,[(s,d) for s,d in jobs if s in common]):
   for hour_ms,b,sl in rows:
    h=index_ms.get(hour_ms);den=b+sl
    if h is not None and den>0:x.at[h,s]=(b-sl)/den
