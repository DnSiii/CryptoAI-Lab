...
OUT=Path('reports/v98_independent_phase105_coinmetrics_txcount_feasibility.json')
BASE='https://community-api.coinmetrics.io/v4/timeseries/asset-metrics'
ASSETS=('btc','eth'); METRIC='TxCnt'; START='2023-01-01'; END='2025-12-31'; EXPECTED=1096

def fetch(asset):
    params={'assets':asset,'metrics':METRIC,'frequency':'1d','start_time':START,'end_time':END,'page_size':10000}
    url=BASE+'?'+urlencode(params)
    req=Request(url,headers={'User-Agent':'CryptoAI-Lab-V98-Independent/phase105'})
    try:
...
def main():
    ...
    report={
        'engine':'V98 Independent','phase':'105','mode':'DATA_ONLY_NO_ALPHA_NO_PNL',
        'source':'Coin Metrics Community API','metric':METRIC,'frequency':'1d',
        'window':[START,END],'expected_days_per_asset':EXPECTED,'assets':assets,'gate':gate,
        'validation':None,'final_holdout':None,'future_holdout_required':True,
        'v16_used':False,'v99_used':False,'phase083_selection_use':False,
        'alpha_or_pnl_computed':False,'parameter_search':False,'rescue_allowed':False,
