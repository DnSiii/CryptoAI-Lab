...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase031_market_decoupling.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase031_market_decoupling_positions.csv'
PHASE={'id':'phase_031_market_decoupling','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'r2_lookback_hours':720,'min_periods':360,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    ...
    ret=lag.pct_change(fill_method=None); btc=ret['BTCUSDT']; var=btc.rolling(c['beta_lookback_hours'],min_periods=c['min_periods']).var(); cov=ret.rolling(c['beta_lookback_hours'],min_periods=c['min_periods']).cov(btc); beta=cov.div(var,axis=0)
    corr=ret.rolling(c['r2_lookback_hours'],min_periods=c['min_periods']).corr(btc); r2=corr.pow(2).clip(0,1); score=0.5-r2.rank(axis=1,pct=True); score=score.where(eligible)
...
def main():
    ...
    hyp='Persistently BTC-decoupled assets, measured by low lagged rolling BTC R-squared, may carry independent cross-sectional return information after beta and dollar neutralization.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase031_market_decoupling_preregistration.md','selection_policy':'single fixed causal market-decoupling specification; no sign/window/cadence/gross/threshold/regime search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
