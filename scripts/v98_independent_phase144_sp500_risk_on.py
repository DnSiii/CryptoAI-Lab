...
import v98_independent_phase143_sp500_data_only as p143
CFG=PROJECT/'config'/'v98_independent.json';PRE=PROJECT/'reports'/'v98_independent_phase144_sp500_risk_on_preregistration.json';P143=PROJECT/'reports'/'v98_independent_phase143_sp500_data_only.json';OUT=PROJECT/'reports'/'v98_independent_phase144_sp500_risk_on.json'
ASSETS=['BTCUSDT','ETHUSDT','BNBUSDT','XRPUSDT','SOLUSDT'];LOOKBACK=20;LAG_DAYS=1;TARGET=.45;CAP=.50

def load_macro():
 raw=p143.acquire(); a=p143.audit(raw); governing=json.loads(P143.read_text()); assert governing['decision']=='PASS_DATA_ONLY'; assert a['normalized_observations_sha256']==governing['normalized_observations_sha256']
 r=csv.DictReader(io.StringIO(raw.decode('utf-8-sig'))); obs=[]
 for row in r:
  v=(row.get('SP500') or '').strip()
...
def main():
 pre=json.loads(PRE.read_text()); assert pre['phase']=='144' and pre['anti_overfit']['parameter_search'] is False and pre['anti_overfit']['rescue_allowed'] is False
 s,h=load_macro(); assert h==pre['dependency']['phase143_observations_sha256']
 ...
 d=daily(base,a,b);pos=d[d>0];top10=float(pos.nlargest(10).sum()/pos.sum()) if float(pos.sum())>0 else 0.;shares=asset_share(data,t,a,b);open_gross=base.open_positions.loc[a:b].abs().sum(axis=1);max_open=float(open_gross.max());max_close=float(base.positions.loc[a:b].abs().sum(axis=1).max())
 fail=[]
 ...
 if max_open>CAP+1e-12:fail.append('max_open_gross>0.50')
 report={'engine':'V98 Independent','phase':'144','hypothesis':'20-observation SP500 risk-on permission for equal-weight long crypto','preregistration':'reports/v98_independent_phase144_sp500_risk_on_preregistration.json','phase143_dependency':{'decision':'PASS_DATA_ONLY','hash':h,'reacquisition_hash_verified':True},'signal_contract':{'lookback_accepted_observations':LOOKBACK,'direction':'SP500_t > SP500_t_minus_20 => long; else flat','economic_use_lag_days':LAG_DAYS,'target_gross':TARGET,'hard_gross_cap':CAP,'assets':ASSETS},'training':train,'folds':folds,'stress_training':stress,'regimes_training':p3.regime_metrics(base,data,a,b,b),'concentration_training':p3.concentration_metrics(base,a,b),'asset_positive_contribution_share':shares,'tails_training':{'top10_positive_day_share':top10,'p01_day':train['p01_day'],'p05_day':train['p05_day'],'cvar05_day':train['cvar05_day'],'worst_day':train['worst_day'],'best_day':train['best_day']},'max_open_gross':max_open,'max_close_gross':max_close,'validation':None,'final_holdout':None,'parameter_search':False,'rescue_allowed':False,'v16_used':False,'v99_used':False,'gate':{'passed':not fail,'decision':'PASS_TRAINING' if not fail else 'REJECT_NO_RESCUE','failures':fail}}
