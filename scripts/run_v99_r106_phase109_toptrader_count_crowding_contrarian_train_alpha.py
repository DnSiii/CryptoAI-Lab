# Phase109 frozen preregistered implementation; train-only execution.
...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase109_toptrader_count_crowding_contrarian_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';FIELD='count_toptrader_long_short_ratio';ALPHA_GROSS=.20

def get(u):
 last=None
 for attempt in range(5):
  try:
   with urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase109'}),timeout=90) as r:return r.read()
...
def main():
 assert (PROJECT/'research'/'v99_r106_phase109_toptrader_count_crowding_contrarian_prereg.md').exists();prev=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase108_global_crowding_contrarian_train_alpha.json').read_text());assert prev['status']=='TRAIN_ALPHA_REJECT' and prev['precommitment']['holdout_not_parsed']
 ...
 z=rz(x);lag=z.shift(1);sig=(-np.tanh(lag)).where(lag.notna().sum(axis=1)>=10,0).fillna(0);targets=sig.div(sig.abs().sum(axis=1).replace(0,np.nan),axis=0).fillna(0)
 ...
 out={'study':'V99 R106 Phase109 — TOP-TRADER ACCOUNT-COUNT CROWDING CONTRARIAN TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase109_toptrader_count_crowding_contrarian_prereg.md','source':'Binance USD-M count_toptrader_long_short_ratio','transform':'negative tanh of cross-sectional median/MAD robust z of log(top-trader account-count long/short ratio), whole alpha shifted t-1; L1 normalized','direction':'top-trader account-count crowding contrarian: high long tilt short, low long tilt long','single_hypothesis_no_grid':True,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'simultaneous_crosssection_required':True,'min_assets':10,'unit_tanh_scale_fixed':True},'train_end':te.isoformat(),'diagnostic':d,'selected_train_only':'native_toptrader_count_crowding_contrarian' if passed else None,'archives_consumed':len(jobs),'next_gate':'PASS freezes exact spec for supersevere/regime/benchmark/reproducibility before untouched holdout; FAIL permanent, no retuning.','quarantined_symbols':quarantined};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
