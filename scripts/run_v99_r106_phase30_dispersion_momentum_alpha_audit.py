...
REPORT=PROJECT/'reports'/'candidate_v99_r106_phase30_dispersion_momentum_alpha_audit.json'
ALPHA_GROSS=.20; TOP_N=2; VOL_LOOKBACK=168
MIN_TRAIN_ACTIVE_HOURS=24*30; MIN_FOLD_ACTIVE_HOURS=24*5; MIN_HOLDOUT_ACTIVE_HOURS=24*10

def weights(score,close):
    sigma=close.pct_change(fill_method=None).rolling(VOL_LOOKBACK,min_periods=96).std().shift(1).replace(0,np.nan)
...
def sleeves(data):
    ...
    return {n:weights(gated(r),c) for n,r in {'dispersion_momentum_24h':r24,'dispersion_momentum_72h':r72,'dispersion_momentum_168h':r168}.items()}
...
def main():
    ...
    out={'study':'V99 R106 phase 30 — cross-sectional dispersion momentum alpha audit','status':'DIAGNOSTIC_ONLY_NO_STRATEGY_CHANGE','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'objective':'test fixed cross-sectional continuation conditional on elevated dispersion after phase29 rejection','families':list(ss),'alpha_gross':ALPHA_GROSS,'top_n_each_side':TOP_N,'all_features_causal_t_minus_1':True,'selection_uses_train_only':True,'holdout_cannot_change_selected_family':True,'no_parameter_grid':True,'cost_for_selection':'severe','strategy_change_in_phase30':False},'data':{'common_start':start.isoformat(),'common_end':end.isoformat(),'train_end':train_end.isoformat(),'holdout_start':hold_start.isoformat(),'severe_cost_per_side':severe},'sleeves':ds,'selected_train_only':selected,'selected_holdout_descriptive':hold,'selected_holdout_pass':hp,'actionable_for_phase31':bool(selected and hp),'quarantined_symbols':quarantined,'metadata':metadata,'disclosure':'Historical research only. No real orders. Phase30 cannot alter V99 Frozen or V16 Frozen.'}
