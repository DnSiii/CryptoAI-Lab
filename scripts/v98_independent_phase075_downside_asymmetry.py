...
CFG=PROJECT/'config'/'v98_independent.json';OUT=PROJECT/'reports'/'v98_independent_phase075_downside_asymmetry.json';POS=PROJECT/'reports'/'v98_independent_phase075_downside_asymmetry_positions.csv'
PREREG='reports/v98_independent_phase075_prereg.md';SYMBOLS=('BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT');GROSS=.75;LOOKBACK_HOURS=504

def targets(data,start,end):
 close=data.close.loc[:end,list(SYMBOLS)];lr=np.log(close).diff().shift(1);down=lr.where(lr<0,0.).pow(2).rolling(LOOKBACK_HOURS,min_periods=LOOKBACK_HOURS).mean();up=lr.where(lr>0,0.).pow(2).rolling(LOOKBACK_HOURS,min_periods=LOOKBACK_HOURS).mean();score=down/(down+up).replace(0,np.nan)
...
def main():
 ...
 active=(t.loc[a:b,list(SYMBOLS)].abs().sum(axis=1)>0);rep={'engine':'V98 Independent','phase':'075','hypothesis':'frozen 21-day cross-sectional downside-risk asymmetry','preregistration':PREREG,'parameters':{'symbols':SYMBOLS,'lookback_hours':LOOKBACK_HOURS,'gross':GROSS,'rebalance':'00:00 UTC daily'},'activation':{'active_hour_ratio':float(active.mean()),'active_hours':int(active.sum()),'hours':int(len(active))},'training':train,'folds':folds,'stress_training':stress,'regimes_training':p3.regime_metrics(base,data,a,b,b),'concentration_training':p3.concentration_metrics(base,a,b),'asset_contribution_proxy':contribution(data,t,a,b),'tails_training':tails(base,a,b),'reproducibility':{'positions_sha256':hashlib.sha256(t.loc[a:b,list(SYMBOLS)].to_csv().encode()).hexdigest(),'prereg_sha256':hashlib.sha256((PROJECT/PREREG).read_bytes()).hexdigest()},'training_gate':{'passed':not fail,'failures':fail},'validation':None,'final_holdout':None,'final_holdout_untouched':True,'v99_used':False,'v16_used':False,'parameter_search':False,'rescue_allowed':False};OUT.write_text(json.dumps(rep,indent=2,default=str)+'\n');base.positions.to_csv(POS);print(json.dumps({'phase':'075','training_gate':rep['training_gate'],'activation':rep['activation'],'training':train,'folds':folds,'stress':stress,'final_holdout':'UNTOUCHED'},indent=2,default=str))
