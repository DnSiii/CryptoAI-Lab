...
def main():
    ...
    out={'study':'V99 R106 Phase135 — RETURN/VOLUME COUPLING CONTINUATION TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase134_rejection_and_phase135_prereg.md','feature':'hourly return * log(dollar_volume/trailing median168); rolling mean72; robust cross-sectional z; tanh continuation; complete alpha t-1; L1','direction':'return_volume_coupling_continuation','state_hours':STATE_WIN,'baseline_hours':BASE_WIN,'min_assets':MIN_ASSETS,'alpha_gross':ALPHA_GROSS,'single_hypothesis_no_grid':True,'selection_train_only':True,'canonical_replay_may_materialize_full_panel':True,'holdout_rows_used_for_feature_construction':0,'holdout_rows_used_for_selection':0,'no_sign_flip':True},'train_end_exclusive':TRAIN_END.isoformat(),'causality_invariants':{'feature_input_max_timestamp':ct.index.max().isoformat(),'feature_inputs_strictly_pre_train_end':bool(ct.index.max()<TRAIN_END),'post_train_targets_zero':True,'full_score_shift_hours':1,'max_l1':float(targets.abs().sum(axis=1).max())},'diagnostic':d,'selected_train_only':'return_volume_coupling_continuation_72_168' if passed else None,'next_gate':'PASS freezes exact spec for supersevere/regime/tails/concentration/benchmark/reproducibility before untouched holdout; FAIL permanent, no retuning.','quarantined_symbols':quarantined,'metadata':metadata}
