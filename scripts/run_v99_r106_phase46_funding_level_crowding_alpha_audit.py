...
PROJECT=Path(__file__).resolve().parents[1]; REPORT=PROJECT/'reports'/'candidate_v99_r106_phase46_funding_level_crowding_alpha_audit.json'; ALPHA_GROSS=.20; MIN_TRAIN_ACTIVE_HOURS=720; MIN_FOLD_ACTIVE_HOURS=120; MIN_HOLDOUT_ACTIVE_HOURS=240

def sleeves(data):
    c=data.close.astype(float); known=data.funding.replace(0.0,np.nan).ffill().shift(1); out={}
    # Pre-registered threshold-free crowding/carry hypothesis: expensive positive funding is short, negative/cheap funding is long.
    # Only three coarse smoothing horizons; no threshold or parameter grid, and all information is known at t-1.
    for h in (24,72,168):
        level=known.rolling(h,min_periods=max(8,h//3)).mean()
        out[f'funding_level_crowding_reversal_{h}h']=p31.weights(-level,c)
    return out
...
def main():
    ...
    out={'study':'V99 R106 phase 46 — funding-level crowding alpha audit','status':'DIAGNOSTIC_ONLY_NO_STRATEGY_CHANGE','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'objective':'test whether lagged cross-sectional funding level carries a crowding premium: short expensive positive funding and long cheap/negative funding','distinction_from_phase37_phase45':'phase37/45 test changes in funding pressure; phase46 tests the funding level itself, with no price interaction','families':list(ss),'alpha_gross':ALPHA_GROSS,'all_features_causal_t_minus_1':True,'selection_uses_train_only':True,'holdout_cannot_change_selected_family':True,'no_parameter_grid':True,'cost_for_selection':'severe','strategy_change_in_phase46':False},'data':{'common_start':start.isoformat(),'common_end':end.isoformat(),'train_end':train_end.isoformat(),'holdout_start':hold_start.isoformat(),'severe_cost_per_side':severe},'sleeves':ds,'selected_train_only':selected,'selected_holdout_descriptive':hold,'selected_holdout_pass':hp,'actionable_for_phase47':bool(selected and hp),'next_phase_policy':'If actionable, freeze exact train-selected sleeve and run severe+supersevere temporal folds, regime matrix and benchmark envelope; otherwise reject without horizon/threshold tuning and move to a distinct mechanism.','quarantined_symbols':quarantined,'metadata':metadata,'disclosure':'Historical research only. No real orders. Phase46 cannot alter V99 Frozen or V16 Frozen.'}; REPORT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n',encoding='utf-8'); print(json.dumps({'selected_train_only':selected,'selected_holdout_pass':hp,'actionable_for_phase47':out['actionable_for_phase47'],'train_summary':{n:{'stable_train':r['stable_train'],'healthy_folds':r['healthy_folds'],'train':r['train']} for n,r in ds.items()}},indent=2,default=audit.safe_float),flush=True)
