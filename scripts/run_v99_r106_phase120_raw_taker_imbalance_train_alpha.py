...
def load(job):
 ...
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  for row in rd:
   ...
   buy=q if bm=='false' else 0.; sell=q if bm=='true' else 0.; a=acc.setdefault(h,[0.,0.]);a[0]+=buy;a[1]+=sell
...
def main():
 ...
 selected=sorted(av['symbols'],key=lambda s:(int(av['symbols'][s]['listed_compressed_bytes']),s))[:N];selected_bytes=sum(int(av['symbols'][s]['listed_compressed_bytes']) for s in selected);jobs=[]
 for s in selected:
 ...
 z=rz(x);sig=np.tanh(z).shift(1);sig=sig.where(sig.notna().sum(axis=1)>=MIN_ASSETS,0).fillna(0);targets=pd.DataFrame(0.,index=data.close.index,columns=data.close.columns);common=[s for s in selected if s in targets.columns];targets.loc[:,common]=sig[common].div(sig[common].abs().sum(axis=1).replace(0,np.nan),axis=0).fillna(0)
