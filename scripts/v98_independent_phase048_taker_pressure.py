...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; AUDIT_PATH=PROJECT/'reports'/'v98_independent_phase047_oi_acquisition_audit.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase048_taker_pressure.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase048_taker_pressure_positions.csv'
PHASE={'id':'phase_048_taker_pressure','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'min_beta_obs':360,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
SYMBOLS=acq.SYMBOLS; BASE=acq.BASE

def canonical_taker(df):
 x=pd.DataFrame({'timestamp':pd.to_datetime(df['create_time'],utc=True,errors='coerce'),'ratio':pd.to_numeric(df['sum_taker_long_short_vol_ratio'],errors='coerce')})
 x=x[x.timestamp.notna() & x.ratio.notna() & (x.ratio>0)].copy().sort_values('timestamp',kind='stable'); x['hour']=x.timestamp.dt.floor('h')
...
def fetch_day(symbol,date):
 ...
 try:
  ...
  with zipfile.ZipFile(io.BytesIO(payload)) as z:
   ...
   with z.open(names[0]) as f: df=pd.read_csv(f,usecols=['create_time','sum_taker_long_short_vol_ratio'])
  h=canonical_taker(df)
  if h.empty:return symbol,date,None,'no_valid_hourly_taker'
  return symbol,date,h[['hour','ratio']],None
...
def acquire(start,end):
 ...
 for s,parts in frames.items():
  if parts: out[s]=pd.concat(parts,ignore_index=True).drop_duplicates('hour',keep='last').sort_values('hour').set_index('hour')['ratio']
...
def build_targets(data,membership,ratio):
 c=PHASE; idx=data.close.index; ratio=ratio.reindex(idx); lag=np.log(ratio.where(ratio>0)).shift(1); ret=data.close.shift(1).pct_change(fill_method=None); btc=ret['BTCUSDT']; var=btc.rolling(c['beta_lookback_hours'],min_periods=c['min_beta_obs']).var(); beta=ret.rolling(c['beta_lookback_hours'],min_periods=c['min_beta_obs']).cov(btc).div(var,axis=0)
...
def main():
 ...
 if audit.get('decision')!='integrity_sample_pass':raise RuntimeError('Phase048 blocked: shared metrics archive integrity sample has not passed')
 ...
 ratio,cov=acquire(cfg['research_start'],cfg['training_end']); t=build_targets(liquid,membership,ratio); r={s:ev(liquid,t,cfg,s) for s in ('base','severe','supersevere')}; a,b=cfg['research_start'],cfg['training_end']; train=p3.metrics(r['base'],a,b); folds={f['name']:p3.metrics(r['base'],f['start'],f['end']) for f in cfg['folds']}; sev=p3.metrics(r['severe'],a,b); sup=p3.metrics(r['supersevere'],a,b); conc=p3.concentration_metrics(r['base'],a,b); beta_n=p3.beta_neutrality_metrics(r['base'],liquid,a,b); regimes=p3.regime_metrics(r['base'],liquid,a,b,b); passed,failures=p3.training_gate(train,folds,sev,sup,conc)
 hyp='Contracts with stronger strictly lagged aggressive taker long/short volume pressure are more crowded and subsequently underperform weaker-pressure peers after dollar/BTC-beta neutralization.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase048_taker_pressure_preregistration.md','acquisition_training':cov,'selection_policy':'single frozen negative lagged taker-ratio signal; no sign/window/cadence/gross/threshold/regime/symbol-subset search','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
 if passed:
  vr0,vcov=acquire(cfg['validation_start'],cfg['validation_end']); allr=pd.concat([ratio,vr0]).sort_index(); vt=build_targets(liquid,membership,allr); vr={s:ev(liquid,vt,cfg,s) for s in ('base','severe','supersevere')}; v0,v1=cfg['validation_start'],cfg['validation_end']; vb=p3.metrics(vr['base'],v0,v1); vs=p3.metrics(vr['severe'],v0,v1); vss=p3.metrics(vr['supersevere'],v0,v1); vp,vf=p3.holdout_gate(vb,vs,vss); report['validation']={'acquisition':vcov,'base':vb,'severe':vs,'supersevere':vss,'gate':{'passed':vp,'failures':vf}}
 REPORT_PATH.write_text(json.dumps(report,indent=2,default=str)+'\n'); r['base'].positions.to_csv(POSITIONS_PATH); validation_pass=bool(passed and report['validation'] and report['validation']['gate']['passed']); state['phase']='phase_048_complete'; state['last_experiment']={'id':PHASE['id'],'hypothesis':hyp,'status':'frozen_pending_final_holdout' if validation_pass else ('training_passed_validation_rejected' if passed else 'training_rejected'),'parameters':'preregistered negative lagged taker ratio; daily rebalance; 720h beta; 0.75 gross; beta/dollar neutral','report':str(REPORT_PATH.relative_to(PROJECT)),'training_summary':train,'folds':folds,'stress_training':report['stress_training'],'training_gate':report['training_gate'],'validation':report['validation'],'final_holdout_opened':False}; state['champion']=PHASE['id'] if validation_pass else state.get('champion'); state['final_holdout_untouched']=True; state['next_action']='If Phase048 passes training+validation, freeze before one-shot final holdout. If rejected, close taker-pressure reversal without rescue tuning.'; STATE_PATH.write_text(json.dumps(state,indent=2,default=str)+'\n'); print(json.dumps({'phase':PHASE['id'],'coverage':cov,'training_gate':report['training_gate'],'training':train,'folds':folds,'stress':report['stress_training'],'validation':report['validation'],'final_holdout':'UNTOUCHED'},indent=2,default=str))
