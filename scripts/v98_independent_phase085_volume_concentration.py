...
CFG=PROJECT/'config'/'v98_independent.json';OUT=PROJECT/'reports'/'v98_independent_phase085_volume_concentration.json';POS=PROJECT/'reports'/'v98_independent_phase085_volume_concentration_positions.csv'
PREREG='reports/v98_independent_phase085_prereg.md';SYMBOLS=('BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT');GROSS=.75;MIN_HOURS=20

def daily_hhi(data):
 v=data.frames['quote_volume'].loc[:,list(SYMBOLS)].where(lambda x:np.isfinite(x)&(x>=0))
 rows=[]
 for day,g in v.groupby(v.index.normalize()):
  r={}
  for c in v.columns:
   x=g[c].dropna();s=float(x.sum())
   r[c]=float(((x/s)**2).sum()) if len(x)>=MIN_HOURS and s>0 else np.nan
  rows.append(pd.Series(r,name=day))
 return pd.DataFrame(rows)

def targets(data,start,end):
 h=daily_hhi(data);t=pd.DataFrame(0.,index=data.close.index,columns=data.close.columns);events=(t.index.hour==0)&(t.index>=pd.Timestamp(start))&(t.index<=pd.Timestamp(end))
 for ts in t.index[events]:
  prior=ts.normalize()-pd.Timedelta(days=1)
  if prior not in h.index: continue
  s=h.loc[prior,list(SYMBOLS)].dropna();ordered=sorted(((float(s[k]),k) for k in s.index),key=lambda x:(x[0],x[1]))
  if len(ordered)<2 or not ordered[-1][0]>ordered[0][0]: continue
  t.loc[ts,ordered[-1][1]]=GROSS/2;t.loc[ts,ordered[0][1]]=-GROSS/2
...
def main():
 ...
 active=(t.loc[a:b,list(SYMBOLS)].abs().sum(axis=1)>0);rep={'engine':'V98 Independent','phase':'085','hypothesis':'frozen lagged completed-day intraday quote-volume HHI concentration spread','preregistration':PREREG,'parameters':{'symbols':SYMBOLS,'volume_field':'quote_volume','min_hours':MIN_HOURS,'gross':GROSS,'rebalance':'00:00 UTC daily'},'phase083_selection_use':False,'future_holdout_required':True,'activation':{'active_hour_ratio':float(active.mean()),'active_hours':int(active.sum()),'hours':int(len(active))},'training':train,'folds':folds,'stress_training':stress,'regimes_training':p3.regime_metrics(base,data,a,b,b),'concentration_training':p3.concentration_metrics(base,a,b),'asset_contribution_proxy':contribution(data,t,a,b),'tails_training':tails(base,a,b),'reproducibility':{'positions_sha256':hashlib.sha256(t.loc[a:b,list(SYMBOLS)].to_csv().encode()).hexdigest(),'prereg_sha256':hashlib.sha256((PROJECT/PREREG).read_bytes()).hexdigest()},'training_gate':{'passed':not fail,'failures':fail},'validation':None,'final_holdout':None,'v99_used':False,'v16_used':False,'parameter_search':False,'rescue_allowed':False};OUT.write_text(json.dumps(rep,indent=2,default=str)+'\n');base.positions.to_csv(POS);print(json.dumps({'phase':'085','training_gate':rep['training_gate'],'training':train,'folds':folds,'stress':stress,'phase083_selection_use':False},indent=2,default=str))
