...
def main() -> None:
    ...
    if config["strict_research_gate"] != "UNVALIDATED":
        raise RuntimeError("V99 validation status was misrepresented")
    if not state.get("experimental") or not ledger.get("experimental"):
        raise RuntimeError("V99 experimental disclosure missing")
    ...
    if ledger.get("track") != "v99" or ledger.get("schema_version") != 4:
        raise RuntimeError("V99 ledger invalid")
