...
import v98_independent_phase152_dff_data_only as p152
CFG=PROJECT/'config'/'v98_independent.json';PRE=PROJECT/'reports'/'v98_independent_phase153_dff_tightening_risk_off_preregistration.json';P152=PROJECT/'reports'/'v98_independent_phase152_dff_data_only.json';OUT=PROJECT/'reports'/'v98_independent_phase153_dff_tightening_risk_off.json'
ASSETS=['BTCUSDT','ETHUSDT','BNBUSDT','XRPUSDT','SOLUSDT'];LOOKBACK=20;LAG_DAYS=1;TARGET=.30;CAP=.35

def load_macro():
 raw=p152.acquire(); a=p152.audit(raw); governing=json.loads(P152.read_text()); assert governing['decision']=='PASS_DATA_ONLY'; assert a['normalized_observations_sha256']==governing['normalized_observations_sha256']
 r=csv.DictReader(io.StringIO(raw.decode('utf-8-sig')));obs=[]
 for row in r:
  v=(row.get('DFF') or '').strip()
...
def main():
 pre=json.loads(PRE.read_text());assert pre['phase']=='153' and pre['anti_overfit']['parameter_search'] is False and pre['anti_overfit']['rescue_allowed'] is False and pre['anti_overfit']['sign_flip_allowed'] is False
 s,h=load_macro();assert h==pre['dependency']['phase152_observations_sha256']
