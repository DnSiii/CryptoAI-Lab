...
REPORT = PROJECT / "reports" / "candidate_v99_r106_phase11_crash_short_continuation_audit.json"
EPS = 1e-12
HORIZON = 3
TRAIN_FOLDS = 4
...
def trim_top(values: np.ndarray, q: float = 0.99) -> float:
    ...
    return float(values[values <= cutoff].mean()) if np.any(values <= cutoff) else 0.0
...
def eval_rule(rule: pd.DataFrame, eligible: pd.DataFrame, future: pd.DataFrame, start, end):
    ...
    base_hit = float((base_vals > 0).mean()) if len(base_vals) else 0.0
    return {
...
def main():
    ...
    close = data.close.reindex(index=pos.index, columns=pos.columns)
    ...
    rules = {
        ...
        "price3_down": r3 < 0.0,
        "price6_down": r6 < 0.0,
        "recent3_positive_and_price3_down": (recent3 > 0.0) & (r3 < 0.0),
        "recent6_positive_and_price6_down": (recent6 > 0.0) & (r6 < 0.0),
    }
    ...
    candidates = [(n, x) for n, x in train.items() if x["selected_asset_hours"] >= 200 and x["coverage"] >= 0.05]
    candidates.sort(key=lambda x: (x[1]["mean_without_top1pct"], x[1]["mean_future_net_pnl"], x[1]["hit_lift"]), reverse=True)
    ...
    srf = [fold_rows[k].get(selected, {}) for k in sorted(fold_rows)] if selected else []
    valid_folds = [r for r in srf if r.get("selected_asset_hours", 0) >= 30]
    positive_folds = sum(int(r.get("mean_without_top1pct", 0.0) > 0.0 and r.get("mean_future_net_pnl", 0.0) > 0.0) for r in valid_folds)
    passed = bool(
        selected
        and sr_train.get("mean_without_top1pct", 0.0) > 0.0
        and sr_hold.get("mean_without_top1pct", 0.0) > 0.0
        and sr_train.get("mean_future_net_pnl", 0.0) > 0.0
        and sr_hold.get("mean_future_net_pnl", 0.0) > 0.0
        and len(valid_folds) >= 3
        and positive_folds >= 3
    )

    out = {
        "study": "V99 R106 phase 11 — crash short continuation audit",
        "status": "DIAGNOSTIC_ONLY_NO_STRATEGY_CHANGE",
        "frozen_assets_untouched": {"v16": True, "v99_frozen": True},
        "precommitment": {
            "context": "R98 short positions during causal crash-continuation state",
            "future_horizon_hours": HORIZON,
            "candidate_rules": list(rules),
            "no_numeric_threshold_grid": True,
            "selection_uses_train_only": True,
            "tail_robustness": "selected rule must retain positive mean after removing top 1% future outcomes in train and holdout",
        },
        ...
        "disclosure": "Diagnostic-only future-label study. No strategy changes or real orders.",
