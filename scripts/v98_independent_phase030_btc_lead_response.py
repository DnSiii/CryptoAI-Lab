...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase030_btc_lead_response.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase030_btc_lead_response_positions.csv'
PHASE={'id':'phase_030_btc_lead_response','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'lead_lookback_hours':720,'driver_hours':24,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    ...
    btc_lead=btc.shift(1); lead_var=btc_lead.rolling(c['lead_lookback_hours'],min_periods=360).var(); lead_beta=hourly.rolling(c['lead_lookback_hours'],min_periods=360).cov(btc_lead).div(lead_var,axis=0)
    driver=lag['BTCUSDT'].pct_change(c['driver_hours'],fill_method=None); raw=lead_beta.mul(driver,axis=0); score=raw.rank(axis=1,pct=True).sub(0.5).where(eligible)
...
def main():
    ...
    hyp='Persistent cross-sectional differences in next-hour response to lagged BTC returns may create a causal lead-response alpha after recent BTC displacement.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase030_btc_lead_response_preregistration.md','selection_policy':'single fixed causal BTC lead-response specification; no sign/horizon/cadence/threshold/regime/leverage search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
