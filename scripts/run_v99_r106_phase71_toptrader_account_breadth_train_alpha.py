...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'reports'/'candidate_v99_r106_phase71_toptrader_account_breadth_train_alpha.json';BASE='https://data.binance.vision/data/futures/um/daily/metrics';ALPHA_GROSS=.20
FIELD='count_toptrader_long_short_ratio'
def get(u):
 req=urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase71'})
...
def main():
 ...
 out={'study':'V99 R106 Phase71 — native top-trader account breadth TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_HOLDOUT' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase71_toptrader_account_breadth_prereg.md','source':'Binance USD-M daily metrics count_toptrader_long_short_ratio','transform':'log(count_toptrader_long_short_ratio).shift(1)','direction':'continuation','single_hypothesis_no_grid':True,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'blank_or_nonfinite_observations_treated_missing':True},'train_end':train_end.isoformat(),'diagnostic':d,'selected_train_only':'native_toptrader_account_breadth' if passed else None,'archives_consumed':len(jobs),'next_gate':'If pass, freeze exact specification and evaluate downstream gates before untouched holdout; if reject, no sign/field/smoothing/threshold retuning.','quarantined_symbols':quarantined,'disclosure':'Phase71 consumes native metrics only through train_end; every consumed archive is SHA256+CRC verified; missing/nonfinite observations remain missing; no holdout metrics/returns are inspected.'};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'archives_consumed':len(jobs),'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
if __name__=='__main__':main()
# Workflow-trigger-only comment; preregistered specification above is unchanged.
