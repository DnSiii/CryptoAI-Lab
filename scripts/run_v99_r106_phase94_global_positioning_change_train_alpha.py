...
OUT=PROJECT/'reports'/'candidate_v99_r106_phase94_global_positioning_change_train_alpha.json'
ALPHA_GROSS=.20
...
def main():
 assert (PROJECT/'research'/'v99_r106_phase94_global_positioning_change_prereg.md').exists()
 prev=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase93_global_positioning_robust_distance_continuation_train_alpha.json').read_text());assert prev['status']=='TRAIN_ALPHA_REJECT' and prev['precommitment']['holdout_not_parsed']
 ...
 # p92.load returns log(count_long_short_ratio). A 24h difference is formed first,
 # then shifted one full bar so position at t can only depend on information <=t-1.
 ...
 out={'study':'V99 R106 Phase94 — GLOBAL POSITIONING 24H CHANGE TRAIN-ONLY alpha gate','status':'TRAIN_ALPHA_PASS_FREEZE_FOR_DOWNSTREAM' if passed else 'TRAIN_ALPHA_REJECT','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'prereg':'research/v99_r106_phase94_global_positioning_change_prereg.md','source':'Binance USD-M daily metrics count_long_short_ratio','transform':'log ratio; 24h difference; shifted t-1; simultaneous median/MAD robust z; raw=-tanh(z); L1 normalized; min 10 assets','direction':'mean-reverting 24h positioning change','single_hypothesis_no_grid':True,'change_horizon_hours':24,'alpha_gross':ALPHA_GROSS,'selection_train_only':True,'holdout_not_parsed':True,'missing_archives_not_filled':True,'simultaneous_crosssection_required':True,'min_assets':10,'unit_tanh_scale_fixed':True},'train_end':train_end.isoformat(),'diagnostic':d,'selected_train_only':'native_global_positioning_24h_change_mean_reversion' if passed else None,'archives_consumed':len(jobs),'next_gate':'If pass, freeze exact specification and evaluate supersevere/regime/benchmark/reproducibility gates before untouched holdout; if reject, no retuning.','quarantined_symbols':quarantined,'disclosure':'Phase94 consumes native metrics only through train_end; archive SHA256+CRC verified by Phase92 loader; positive finite ratio; 24h log change then feature shifted t-1; simultaneous median/MAD >=10 assets; fixed -tanh(z), no tuning; no holdout inspected.'};OUT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n');print(json.dumps({'status':out['status'],'archives_consumed':len(jobs),'healthy_folds':d['healthy_folds'],'valid_folds':d['valid_folds'],'train':d['train']},indent=2,default=audit.safe_float))
