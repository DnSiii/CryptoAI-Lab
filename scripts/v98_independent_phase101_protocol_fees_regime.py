...
CFG=PROJECT/'config'/'v98_independent.json';OUT=PROJECT/'reports'/'v98_independent_phase101_protocol_fees_regime.json';POS=PROJECT/'reports'/'v98_independent_phase101_protocol_fees_regime_positions.csv';PREREG='reports/v98_independent_phase101_protocol_fees_prereg.md'
URL='https://api.llama.fi/overview/fees?excludeTotalDataChartBreakdown=true&excludeTotalDataChart=false';SYMBOLS=('BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT');GROSS=.75

def fees():
 req=urllib.request.Request(URL,headers={'Accept':'application/json','User-Agent':'CryptoAI-Lab-V98-Phase101/1.0'});raw=urllib.request.urlopen(req,timeout=60).read();obj=json.loads(raw);rows=[]
