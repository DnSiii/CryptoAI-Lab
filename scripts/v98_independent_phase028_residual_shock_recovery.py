...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase028_residual_shock_recovery.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase028_residual_shock_recovery_positions.csv'
PHASE={'id':'phase_028_residual_shock_recovery','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'shock_hours':24,'scale_lookback_hours':720,'rebalance_hours':24,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    ...
    residual=hourly.sub(beta.mul(btc,axis=0)); shock=residual.rolling(c['shock_hours'],min_periods=c['shock_hours']).sum(); scale=residual.rolling(c['scale_lookback_hours'],min_periods=360).std()*np.sqrt(c['shock_hours']); z=shock.div(scale.replace(0,np.nan)); score=(-z).rank(axis=1,pct=True).sub(0.5).where(eligible)
...
def main():
    ...
    hyp='Lagged standardized 24h BTC-residual shocks may mean-revert cross-sectionally as temporary forced-flow dislocations normalize.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase028_preregistration.md','selection_policy':'single fixed shock-recovery specification; no lookback/sign/cadence/threshold/regime/leverage search; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
    ...
    REPORT_PATH.parent.mkdir(parents=True,exist_ok=True); REPORT_PATH.write_text(json.dumps(report,indent=2,default=str)+'\n'); r['base'].positions.to_csv(POSITIONS_PATH); validation_pass=bool(passed and report['validation'] and report['validation']['gate']['passed']); state['phase']='phase_028_complete'; state['last_experiment']={'id':PHASE['id'],'hypothesis':hyp,'status':'frozen_pending_final_holdout' if validation_pass else ('training_passed_validation_rejected' if passed else 'training_rejected'),'parameters':'preregistered 24h standardized BTC-residual shock recovery; 720h beta/scale; 24h rebalance; 0.75 gross; beta/dollar neutral','report':str(REPORT_PATH.relative_to(PROJECT)),'training_summary':train,'folds':folds,'stress_training':report['stress_training'],'training_gate':report['training_gate'],'validation':report['validation'],'final_holdout_opened':False}; state['champion']=PHASE['id'] if validation_pass else state.get('champion'); state['final_holdout_untouched']=True; state['next_action']='If Phase028 passes training+validation, freeze before one-shot final holdout. If rejected, close residual shock-recovery without sign/lookback/cadence rescue and move to another orthogonal alpha.'; STATE_PATH.write_text(json.dumps(state,indent=2,default=str)+'\n'); print(json.dumps({'phase':PHASE['id'],'training_gate':report['training_gate'],'training':train,'folds':folds,'stress':report['stress_training'],'validation':report['validation'],'final_holdout':'UNTOUCHED'},indent=2,default=str))
