...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT / "reports" / "opportunity_neighborhood_v1.json",
    )
...
def neighborhood(base: StrategySpec) -> list[tuple[str, StrategySpec]]:
    return [
        ...
        ("phase_1", replace(base, rebalance_phase=1)),
        ("phase_2", replace(base, rebalance_phase=2)),
...
def main() -> None:
    ...
    if selected is None:
        raise RuntimeError("budget refinement did not select a candidate")
    finalist = json.loads(
    ...
    report = {
        ...
        "method": (
            "One-factor-at-a-time causal neighborhood around the selected "
            "historical seed. Every row receives all mandatory exact scenarios."
        ),
        "rows": [],
        "neighborhood_gate": None,
        "paper_promotion": False,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)

    for number, (name, spec) in enumerate(neighborhood(seed), 1):
        print(f"neighborhood {number}/11 {name}", flush=True)
        opportunity = build_targets(signal_data, spec)
    ...
    worst_robust_score = min(row["robust_wealth_score"] for row in report["rows"])
    no_ruin = all(row["gates"]["no_exact_ruin"] for row in report["rows"])
    report["neighborhood_gate"] = {
        "passing_fraction": passing_fraction,
        "minimum_required_passing_fraction": 0.70,
        "worst_robust_wealth_score": worst_robust_score,
        "minimum_required_worst_robust_wealth_score": 0.90,
        "no_exact_ruin_anywhere": no_ruin,
        "passes": (
            passing_fraction >= 0.70
            and worst_robust_score >= 0.90
            and no_ruin
        ),
        "meaning": (
            "Passing only permits a separate forward paper proposal. It does "
            "not authorize paper activation or capital."
        ),
    }
    report["status"] = "completed"
    args.output.write_text(json.dumps(report, indent=2) + "\n")
    print(json.dumps(report["neighborhood_gate"], indent=2), flush=True)
