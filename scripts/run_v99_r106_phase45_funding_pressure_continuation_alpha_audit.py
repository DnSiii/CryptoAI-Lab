...
PROJECT=Path(__file__).resolve().parents[1]; REPORT=PROJECT/'reports'/'candidate_v99_r106_phase45_funding_pressure_continuation_alpha_audit.json'; ALPHA_GROSS=.20; MIN_TRAIN_ACTIVE_HOURS=720; MIN_FOLD_ACTIVE_HOURS=120; MIN_HOLDOUT_ACTIVE_HOURS=240

def sleeves(data):
    c=data.close.astype(float); known=data.funding.replace(0.0,np.nan).ffill().shift(1); out={}
    # Pre-registered sign-complement to phase37: test persistence, not reversal, of lagged funding-pressure changes.
    # Horizons are inherited exactly from phase37; no threshold/grid search.
    for fast,slow in ((24,168),(72,336),(168,720)):
        f=known.rolling(fast,min_periods=max(8,fast//3)).mean(); s=known.rolling(slow,min_periods=max(24,slow//3)).mean(); score=f-s
        out[f'funding_pressure_continuation_{fast}v{slow}h']=p31.weights(score,c)
    return out
...
def main():
    ...
    out={'study':'V99 R106 phase 45 — funding-pressure continuation alpha audit','status':'DIAGNOSTIC_ONLY_NO_STRATEGY_CHANGE','frozen_assets_untouched':{'v16':True,'v99_frozen':True},'precommitment':{'objective':'test persistence of causal lagged funding-pressure change using the exact phase37 horizon families with opposite economic sign','distinction_from_phase37':'phase37 reversed fast-minus-slow funding pressure; phase45 tests continuation with identical horizons and no new tuning','families':list(ss),'alpha_gross':ALPHA_GROSS,'all_features_causal_t_minus_1':True,'selection_uses_train_only':True,'holdout_cannot_change_selected_family':True,'no_parameter_grid':True,'cost_for_selection':'severe','strategy_change_in_phase45':False},'data':{'common_start':start.isoformat(),'common_end':end.isoformat(),'train_end':train_end.isoformat(),'holdout_start':hold_start.isoformat(),'severe_cost_per_side':severe},'sleeves':ds,'selected_train_only':selected,'selected_holdout_descriptive':hold,'selected_holdout_pass':hp,'actionable_for_phase46':bool(selected and hp),'next_phase_policy':'If actionable, freeze exact train-selected sleeve and run severe+supersevere temporal folds, regime matrix and benchmark envelope; otherwise reject without horizon/threshold tuning and move to a distinct mechanism.','quarantined_symbols':quarantined,'metadata':metadata,'disclosure':'Historical research only. No real orders. Phase45 cannot alter V99 Frozen or V16 Frozen.'}; REPORT.write_text(json.dumps(out,indent=2,default=audit.safe_float)+'\n',encoding='utf-8'); print(json.dumps({'selected_train_only':selected,'selected_holdout_pass':hp,'actionable_for_phase46':out['actionable_for_phase46'],'train_summary':{n:{'stable_train':r['stable_train'],'healthy_folds':r['healthy_folds'],'train':r['train']} for n,r in ds.items()}},indent=2,default=audit.safe_float),flush=True)
