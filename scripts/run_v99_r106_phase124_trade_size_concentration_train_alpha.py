# Phase124 preregistered unsigned trade-size participation concentration; train-only, holdout prohibited.
...
OUT=PROJECT/'reports'/'candidate_v99_r106_phase124_trade_size_concentration_train_alpha.json'
...
def get(u):
 ...
 for attempt in range(6):
  try:
   with urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'CryptoAI-v99-r106-phase124'}),timeout=180) as r:return r.read()
...
def load(job):
 ...
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  for row in csv.reader(io.TextIOWrapper(z.open(names[0]),encoding='utf-8')):
   if not row:continue
   try:p=float(row[1]);q=float(row[2]);ts=int(row[5])
   except (ValueError,TypeError,IndexError):continue
   if not (np.isfinite(p) and np.isfinite(q) and p>0 and q>0):continue
   h=ts//3600000
   if h*3600000>=int(TRAIN_END.timestamp()*1000):raise RuntimeError('holdout timestamp encountered '+stem)
   v=p*q
   sm,ss=acc.get(h,(0.,0.));acc[h]=(sm+v,ss+v*v)
 return s,[(h,ss/(sm*sm)) for h,(sm,ss) in acc.items() if sm>0]
...
def main():
 assert (PROJECT/'research'/'v99_r106_phase124_trade_size_concentration_prereg.md').exists()
 p123=json.loads((PROJECT/'reports'/'candidate_v99_r106_phase123_aggressor_run_imbalance_train_alpha.json').read_text());assert p123['status']=='TRAIN_ALPHA_REJECT'
 ...
 with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
  for s,rows in pool.map(load,[(s,d) for s,d in jobs if s in common]):
   for h,hhi in rows:
    t=epoch_to_index.get(h)
    if t is not None:x.at[t,s]=hhi
