...
REPORT = PROJECT / "reports" / "candidate_v99_r40_tail_lift_hedge.json"
...
TRIGGER_PRESETS = (
    ...
    {
        "name": "balanced_q90_net",
        "net_abs": 1.1337165946188033,
        "btc_abs3": 0.01805751418811874,
        "breadth_abs2": 0.0661764705882353,
        "cross_vol": 0.012670447074253053,
    },
    {
        "name": "severe_q90_net_q95_turbulence",
        "net_abs": 1.1337165946188033,
        "btc_abs3": 0.025503851371036057,
        "breadth_abs2": 0.13970588235294118,
        "cross_vol": 0.015854923020523555,
    },
)


def tail_gate(targets: pd.DataFrame, close: pd.DataFrame, p: dict):
    ...
    net = t.sum(axis=1)
    net_abs = net.abs()
    btc_abs3 = c["BTCUSDT"].pct_change(3, fill_method=None).abs().fillna(0.0)
    hret = c.pct_change(fill_method=None)
    breadth = (hret.abs() >= 0.02).mean(axis=1)
    cross_vol = hret.std(axis=1).fillna(0.0)
    turbulence = (
        (btc_abs3 >= p["btc_abs3"])
        | (breadth >= p["breadth_abs2"])
        | (cross_vol >= p["cross_vol"])
    )
    instant = ((net_abs >= p["net_abs"]) & turbulence).fillna(False)
    active = instant.astype(float).rolling(int(p["cooldown"]), min_periods=1).max().gt(0.0)
    return active, net, {
        "instant_fraction": float(instant.mean()),
        "active_fraction": float(active.mean()),
        "net_threshold": p["net_abs"],
        "btc_abs3_threshold": p["btc_abs3"],
        "breadth_threshold": p["breadth_abs2"],
        "cross_vol_threshold": p["cross_vol"],
        "net_median_active": float(net_abs.loc[active].median()) if active.any() else 0.0,
        "btc_abs3_median_active": float(btc_abs3.loc[active].median()) if active.any() else 0.0,
    }


def build_candidate(data, raw, ex, guard, gross, cost, p):
    r30_result, r30_targets, _ = r37.build_r30(data, raw, ex, guard, gross, cost)
    active, net, diag = tail_gate(r30_targets, data.close, p)
    direction = -np.sign(net).where(net.abs() >= p["net_abs"], 0.0)
    targets = r30_targets.copy()
    targets["BTCUSDT"] = (
        targets["BTCUSDT"]
        + active.astype(float) * direction * float(p["extra_hedge"])
    )
    targets = cap(targets, float(p["gross_cap"]))
    result = r36.run(data, targets, ex, cost, float(p["gross_cap"]), guard)
    return result, r30_result, targets, active, diag
...
def main():
    ...
    r30_base, _, r30_active = r37.build_r30(data, raw, ex, guard, gross, base_cost)
    ...
    rows = []
    for preset, extra_hedge, cooldown in itertools.product(
        TRIGGER_PRESETS, (0.10, 0.20, 0.30), (3, 6, 12)
    ):
        p = {
            **preset,
            "extra_hedge": extra_hedge,
            "cooldown": cooldown,
            "gross_cap": 1.90,
        }
        ...
        score = (
            ...
            + 5.0 * np.log(max(hold_r30, 1e-12))
            + 14.0 * max(0.0, 1.0 - dd_env)
            - 18.0 * max(0.0, dd_env - 1.0)
            + 12.0 * max(0.0, 1.0 - worst_env)
    ...
    for row in rows[:9]:
        ...
        sev_result, _, _, sev_active, sev_diag = build_candidate(data, raw, ex, guard, gross, severe_cost, p)
        ...
        dominant = bool(
            ...
            and full_risk_pass
            and row["holdout_wealth_ratio_to_r30"] >= 1.02
    ...
    out = {
        "study": "V99 R40 R30 tail-lift adaptive net hedge",
        "status": "RESEARCH_ONLY_DO_NOT_REWRITE_FROZEN_V99_PAPER",
        "objective": "retain the strongest R30 defensive frontier and add a rare opposite-net BTC hedge only when chronologically trained R39 precursor thresholds show both high net exposure and market turbulence",
        "r30_fixed_base": r37.R30_BASE,
        "r30_base_summary": r30_stats,
        "r30_base_severe": r30_severe_stats,
        "r30_hedge_active_fraction": float(r30_active.mean()),
        "threshold_source": "R39 first-60%-chronological quantiles; thresholds are fixed before this candidate grid",
        "trigger_presets": TRIGGER_PRESETS,
        "grid_policy": "27 predeclared combinations = 3 fixed precursor presets x 3 extra hedge sizes x 3 cooldowns; validation horizons are not optimizer inputs",
        "grid_size": len(rows),
        ...
        "finalists": finalists,
        "all_screened": rows,
        "disclosure": "Historical research only. Net exposure, BTC 3h absolute movement, cross-sectional breadth and volatility are known at close t and alter only the next requested target. R39 thresholds were estimated on the first 60% chronological sample before this 27-combination candidate grid. No real-order path is enabled. Any historical winner requires a frozen forward-paper period before promotion.",
