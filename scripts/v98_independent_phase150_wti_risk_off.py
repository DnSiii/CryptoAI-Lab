...
import v98_independent_phase149_wti_data_only as p149
CFG=PROJECT/'config'/'v98_independent.json';PRE=PROJECT/'reports'/'v98_independent_phase150_wti_risk_off_preregistration.json';P149=PROJECT/'reports'/'v98_independent_phase149_wti_data_only.json';OUT=PROJECT/'reports'/'v98_independent_phase150_wti_risk_off.json'
ASSETS=['BTCUSDT','ETHUSDT','BNBUSDT','XRPUSDT','SOLUSDT'];LOOKBACK=20;LAG_DAYS=1;TARGET=.30;CAP=.35

def load_macro():
 raw=p149.acquire(); a=p149.audit(raw); governing=json.loads(P149.read_text()); assert governing['decision']=='PASS_DATA_ONLY'; assert a['normalized_observations_sha256']==governing['normalized_observations_sha256']
 r=csv.DictReader(io.StringIO(raw.decode('utf-8-sig'))); obs=[]
 for row in r:
  v=(row.get('DCOILWTICO') or '').strip()
  if v in ('','.','NA','NaN'):continue
  try:x=Decimal(v); assert x.is_finite() and x>0
...
def targets(data,s):
 # No macro imputation/carry-forward: signal exists only on an accepted WTI observation date,
 # then becomes economically usable exactly one calendar day later. All other days are flat.
...
def main():
 pre=json.loads(PRE.read_text()); assert pre['phase']=='150' and pre['anti_overfit']['parameter_search'] is False and pre['anti_overfit']['rescue_allowed'] is False
 s,h=load_macro(); assert h==pre['dependency']['phase149_observations_sha256']
