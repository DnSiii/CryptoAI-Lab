#!/usr/bin/env python3
"""V98 Independent Phase145 VIXCLS DATA_ONLY integrity auditor.
...
SERIES="VIXCLS"; START=date(2023,1,1); END=date(2025,12,31)
...
def acquire():
 ...
 for attempt in range(4):
  try:
   req=Request(URL,headers={"User-Agent":"CryptoAI-Lab-V98-Independent-Phase145/1.0","Accept":"text/csv","Connection":"close"})
   with urlopen(req,timeout=90) as r:return r.read()
  except Exception as exc:
   last=exc
   if attempt<3:time.sleep(2**attempt)
 raise RuntimeError(f"Phase145 transport failed after retries: {type(last).__name__}") from last
def audit(raw):
