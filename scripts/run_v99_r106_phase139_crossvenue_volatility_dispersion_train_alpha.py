...
OUT=PROJECT/'reports'/'candidate_v99_r106_phase139_crossvenue_volatility_dispersion_train_alpha.json'
PH136=PROJECT/'reports'/'candidate_v99_r106_phase136_okx_crossvenue_data_audit.json'
DEC138=PROJECT/'docs'/'v99_r106_phase138_train_rejection.md'
PREREG=PROJECT/'docs'/'v99_r106_phase139_crossvenue_volatility_dispersion_preregister.md'
ADDENDUM=PROJECT/'docs'/'v99_r106_phase139_rv_definition_addendum.md'
...
LOOKBACK=168;RV_WINDOW=24;ALPHA_GROSS=.20
...
def exact_mad(frame,window):
    return frame.rolling(window,min_periods=window).apply(lambda x: float(np.median(np.abs(x-np.median(x)))),raw=True)

def rv24(close):
    r=np.log(close).diff()
    return np.sqrt(r.pow(2).rolling(RV_WINDOW,min_periods=RV_WINDOW).sum())

def main():
    assert DEC138.exists() and 'TRAIN_ALPHA_REJECT' in DEC138.read_text()
    pt=PREREG.read_text();at=ADDENDUM.read_text();assert 'direction as **reversion**' in pt and '168h' in pt and 'sqrt(sum_' in at and 'BEFORE ANY PHASE139 PNL' in at
    ...
    idx=pd.date_range(TRAIN_START,TRAIN_END-pd.Timedelta(hours=1),freq='h',tz='UTC');okx_close=pd.DataFrame(okx).sort_index();assert okx_close.index.equals(idx)
    cfg,data,raw,ex,guard,gross,quarantined,metadata=p1.r98.r36.v15_setup();c=data.close.astype(float)
    bclose=c.loc[(c.index>=TRAIN_START)&(c.index<TRAIN_END),list(MAP)].reindex(idx);binance_integrity={}
    for sym in MAP:
    ...
    gap=rv24(okx_close)-rv24(bclose);med=gap.rolling(LOOKBACK,min_periods=LOOKBACK).median();mad=exact_mad(gap,LOOKBACK);robust=(gap-med)/(1.4826*mad).where(mad>1e-12)
    ...
    weights=score.div(score.abs().sum(axis=1).replace(0,np.nan),axis=0).fillna(0);targets=pd.DataFrame(0.,index=c.index,columns=c.columns);common=targets.index.intersection(weights.index);targets.loc[common,list(MAP)]=weights.loc[common,list(MAP)]
