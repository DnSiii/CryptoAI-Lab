#!/usr/bin/env python3
"""V98 Independent Phase139 corrected DATA_ONLY integrity auditor v3.
...
v3 changes are transport/parser only: resilient retries and the actual FRED graph CSV
date column name (observation_date). Frozen source/series/window/gates are unchanged.
"""
...
def acquire()->bytes:
    ...
    raise RuntimeError(f"Phase139 transport failed after retries: {type(last).__name__}") from last

def audit(raw:bytes)->dict:
    text=raw.decode("utf-8-sig")
    reader=csv.DictReader(io.StringIO(text))
    if reader.fieldnames != [DATE_FIELD,SERIES]:
        raise RuntimeError(f"unexpected schema: {reader.fieldnames!r}")
    obs=[]; malformed=0; missing=0
    for row in reader:
        try:
            d=datetime.strptime(row[DATE_FIELD],"%Y-%m-%d").date()
        except Exception:
...
def main():
    ...
    a["reproducibility"]={
      ...
      "identical_integrity_metadata":(
        a["counts"]==b["counts"] and a["coverage"]==b["coverage"] and a["gates"]==b["gates"] and a["schema"]==b["schema"]
      )
    }
    if not all(a["reproducibility"].values()):
        a["decision"]="FAIL"
