...
CONFIG_PATH=PROJECT/'config'/'v98_independent.json'; STATE_PATH=PROJECT/'state'/'v98_independent_state.json'; REPORT_PATH=PROJECT/'reports'/'v98_independent_phase026_residual_trend_efficiency.json'; POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase026_residual_trend_efficiency_positions.csv'
PHASE={'id':'phase_026_residual_trend_efficiency','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'beta_lookback_hours':720,'signal_lookback_hours':720,'rebalance_hours':48,'gross_target':0.75,'gross_cap':0.75}
def build_targets(data,membership):
    """Preregistered signed residual displacement/path-length efficiency; all inputs lagged, fixed 30d windows, no tuning."""
    ...
    residual=hourly.sub(beta.mul(btc,axis=0)); num=residual.rolling(c['signal_lookback_hours'],min_periods=360).sum(); den=residual.abs().rolling(c['signal_lookback_hours'],min_periods=360).sum().replace(0,np.nan); efficiency=num.div(den); score=efficiency.rank(axis=1,pct=True).sub(0.5).where(eligible)
...
def main():
    ...
    hyp='Signed lagged BTC-residual displacement divided by residual path length may isolate persistent cross-sectional trends while penalizing noisy momentum.'; report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hyp,'preregistration':'reports/v98_independent_phase026_preregistration.md','selection_policy':'fixed before execution; no grid/sign/cadence/regime/leverage tuning; validation only if training passes; final holdout untouched','training':train,'folds':folds,'stress_training':{'severe':sev,'supersevere':sup},'concentration_training':conc,'beta_neutrality_training':beta_n,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'validation':None,'final_holdout':None}
    if passed:
    ...
    REPORT_PATH.parent.mkdir(parents=True,exist_ok=True); REPORT_PATH.write_text(json.dumps(report,indent=2,default=str)+'\n'); r['base'].positions.to_csv(POSITIONS_PATH); validation_pass=bool(passed and report['validation'] and report['validation']['gate']['passed']); state['phase']='phase_026_complete'; state['last_experiment']={'id':PHASE['id'],'hypothesis':hyp,'status':'frozen_pending_final_holdout' if validation_pass else ('training_passed_validation_rejected' if passed else 'training_rejected'),'parameters':'preregistered 30d BTC beta + 30d signed residual trend efficiency; 48h rebalance; 0.75 gross; beta/dollar neutral','report':str(REPORT_PATH.relative_to(PROJECT)),'training_summary':train,'folds':folds,'stress_training':report['stress_training'],'training_gate':report['training_gate'],'validation':report['validation'],'final_holdout_opened':False}; state['champion']=PHASE['id'] if validation_pass else state.get('champion'); state['final_holdout_untouched']=True; state['next_action']='If Phase026 passes training+validation, freeze before one-shot final holdout. If rejected, close residual trend-efficiency without parameter rescue and move to another orthogonal alpha.'; STATE_PATH.write_text(json.dumps(state,indent=2,default=str)+'\n'); print(json.dumps({'phase':PHASE['id'],'training_gate':report['training_gate'],'training':train,'folds':folds,'stress':report['stress_training'],'validation':report['validation'],'final_holdout':'UNTOUCHED'},indent=2,default=str))
