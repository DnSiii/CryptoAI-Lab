...
REPORT_PATH=PROJECT/'reports'/'v98_independent_phase007_residual_reversal.json'
POSITIONS_PATH=PROJECT/'reports'/'v98_independent_phase007_residual_reversal_positions.csv'
PHASE={'id':'phase_007_residual_reversal','liquidity_top_n':10,'liquidity_lookback_hours':720,'minimum_history_hours':2160,'reversal_hours':72,'volatility_lookback_hours':720,'rebalance_hours':24,'long_count':3,'short_count':3,'gross_cap':0.75}

def build_targets(data,membership):
    """Orthogonal alpha-family test: beta-neutral 3-day residual reversal.
    Phase006 showed persistent residual momentum breaks in 2025 and under costs. Rather
    than tune momentum horizons, Phase007 tests whether short-horizon idiosyncratic
    overshoots mean-revert. All features use t-1 prices via residual_components.
    One predeclared architecture, no grid and no holdout selection.
    """
    hourly,beta,residual,eligible=p5.residual_components(data,membership)
    shock=residual.rolling(PHASE['reversal_hours'],min_periods=PHASE['reversal_hours']).sum()
    score=(-shock).rank(axis=1,pct=True,method='average').where(eligible)
...
def main():
    ...
    hypothesis='Short-horizon idiosyncratic overshoots mean-revert cross-sectionally after BTC beta removal, providing an alpha family orthogonal to failed persistent momentum.'
    report={'engine':'V98 Independent','phase':PHASE,'hypothesis':hypothesis,'selection_policy':'single predeclared residual-reversal architecture; training/folds/stress only; no holdout selection','training':train,'folds':folds,'stress_training':{'severe':severe,'supersevere':supersevere},'concentration_training':conc,'beta_neutrality_training':beta,'regimes_training':regimes,'training_gate':{'passed':passed,'failures':failures},'holdout':None}
    if passed:
    ...
    final=bool(passed and report['holdout'] and report['holdout']['gate']['passed']); state['phase']='phase_007_complete'; state['champion']=PHASE['id'] if final else state.get('champion')
    state['last_experiment']={'id':PHASE['id'],'hypothesis':hypothesis,'status':'validated_candidate' if final else ('training_passed_holdout_rejected' if passed else 'training_rejected'),'parameters':'single predeclared residual-reversal architecture; no grid','report':str(REPORT_PATH.relative_to(PROJECT)),'training_summary':train,'folds':folds,'stress_training':report['stress_training'],'training_gate':report['training_gate'],'holdout_validation_recorded_but_not_for_selection':report['holdout']}
    state['holdout_evaluated_for_validation']=bool(passed); state['next_action']='Diagnose Phase007 from training evidence. If rejected, move to another orthogonal alpha family/portfolio construction; do not tune reversal horizon. If validated, run frozen robustness gate.'; STATE_PATH.write_text(json.dumps(state,indent=2,default=str)+'\n')
