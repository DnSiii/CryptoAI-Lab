...
def test_recent_v98_hypotheses_are_future_invariant():
    ...
    for mod in (P009,P010,P011,P012,P013,P037,P041):
...
def test_v98_phase037_constraints_and_clock():
    ...
    if active.any(): assert float(t.loc[active].sum(axis=1).abs().max())<1e-8
def test_v98_phase041_constraints_and_neutrality():
    d=synthetic_data(); t=P041.build_targets(d,membership_for(d)); assert float(t.abs().sum(axis=1).max())<=P041.PHASE['gross_cap']+1e-12
    changed=t.diff().abs().sum(axis=1)>1e-12; hours=np.flatnonzero(changed.to_numpy()); assert not len(hours) or all(h%P041.PHASE['rebalance_hours']==0 for h in hours)
    active=t.abs().sum(axis=1)>1e-8
    if active.any(): assert float(t.loc[active].sum(axis=1).abs().max())<1e-8
