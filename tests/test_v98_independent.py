...
def synthetic_data(hours=6500):
    ...
    c=pd.DataFrame(close,index=index); o=c.shift(1).fillna(c.iloc[0]); qv={s:1e6*(len(symbols)-i)*(1+.15*np.sin(np.arange(hours)/24+i)) for i,s in enumerate(symbols)}; frames={'open':o,'high':pd.DataFrame(np.maximum(o,c)*1.002,index=index,columns=symbols),'low':pd.DataFrame(np.minimum(o,c)*.998,index=index,columns=symbols),'close':c,'volume':pd.DataFrame(1000.,index=index,columns=symbols),'quote_volume':pd.DataFrame(qv,index=index),'trades':pd.DataFrame(1000.,index=index,columns=symbols)}; funding=pd.DataFrame({s:.0001*np.sin(np.arange(hours)/8+i) for i,s in enumerate(symbols)},index=index); return FuturesData(frames,funding,symbols)
...
def test_recent_v98_hypotheses_are_future_invariant():
    ...
    for mod in (P009,P010,P011,P012,P013,P037):
