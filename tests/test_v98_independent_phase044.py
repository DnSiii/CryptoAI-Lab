...
P044=load_module('v98_independent_phase044','scripts/v98_independent_phase044_volatility_of_volatility.py')
def synthetic_data(hours=6500):
 index=pd.date_range('2024-01-01',periods=hours,freq='h',tz='UTC'); symbols=('BTCUSDT','ETHUSDT','BNBUSDT','XRPUSDT','ADAUSDT','LINKUSDT','LTCUSDT','DOGEUSDT','SOLUSDT','AVAXUSDT'); rng=np.random.default_rng(44); btc_r=rng.normal(.00002,.007,hours); close={'BTCUSDT':100*np.exp(np.cumsum(btc_r))}
...
def test_phase044_future_invariant():
 d=synthetic_data(); cut=5600; a=P044.build_targets(d,membership_for(d)); m=mutate_future(d,cut); b=P044.build_targets(m,membership_for(m)); pd.testing.assert_frame_equal(a.iloc[:cut+1],b.iloc[:cut+1])
def test_phase044_constraints_neutrality_and_clock():
 d=synthetic_data(); t=P044.build_targets(d,membership_for(d)); assert float(t.abs().sum(axis=1).max())<=P044.PHASE['gross_cap']+1e-12; changed=t.diff().abs().sum(axis=1)>1e-12; hours=np.flatnonzero(changed.to_numpy()); assert not len(hours) or all(h%P044.PHASE['rebalance_hours']==0 for h in hours); active=t.abs().sum(axis=1)>1e-8
 if active.any(): assert float(t.loc[active].sum(axis=1).abs().max())<1e-8
def test_phase044_has_no_final_holdout_access():
 text=(PROJECT/'scripts'/'v98_independent_phase044_volatility_of_volatility.py').read_text(); forbidden=("cfg['final_holdout_start']","cfg[\"final_holdout_start\"]","cfg['final_holdout_end']","cfg[\"final_holdout_end\"]"); assert not any(token in text for token in forbidden)
